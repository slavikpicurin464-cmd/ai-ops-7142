...
LATAM_PRESET — sub-profile поверх вертикали для BR/MX/AR/CL/CO/PE (волна Т.14 после Т.14-А создателя)
═══════════════════════════════════════════════════

Контекст: «LATAM-рынок post-2024» (BR / MX / AR / CL / CO / PE) — это **горизонтальный sub-profile**, который накладывается поверх вертикали (CRISIS_EXPERT / HIGH_TICKET_PRO_SERVICES / REAL_ESTATE_EXPAT / INFOBIZ / LOCAL_SERVICE / EDTECH / ECOM_IMPULSE / B2B_SAAS), а не заменяет её. До волны Т.14 покрытие региона было фрагментарным: упоминания LATAM в currency-таблице (BRL/MXN/ARS/CLP/COP/PEN) и точечные cross-ref на WhatsApp Business API. Для LATAM-реальности агент терял восемь критических слоёв: (1) LGPD Art.7+11 + ANPD enforcement (BR), LFPDPPP + INAI (MX), Ley 25.326 + AAIP (AR), Ley 19.628 / Ley 21.719 (CL, effective Dec 2026), Habeas Data (CO Ley 1581/2012), Ley 29733 (PE) — Latin DPL mosaic, (2) consumer-protection регуляторы (CDC BR / PROFECO MX / SERNAC CL / Defensa del Consumidor AR / SIC CO / INDECOPI PE) + ANATEL/IFT subscription rules, (3) health/cosmetics/supplements регуляторы (Anvisa BR / COFEPRIS MX / ANMAT AR / ISP CL / INVIMA CO / DIGEMID PE) — Latin-only approval matrices ≠ US-FDA pattern, (4) financial регуляторы (BACEN+CVM BR / CONDUSEF+CNBV MX / CMF CL / SFC CO / SBS PE) + crypto rules (BR Lei 14.478/2022 Marco Cripto, MX Ley Fintech 2018, AR PSAV registry 2024), (5) language disambiguation BR=PT-BR vs остальные=ES с regional dialect detection (MX-neutral / AR rioplatense / CL chileno / CO costeño-paisa-rolo / PE limeño), (6) cultural / religious sensitivity (high Catholic + Evangelical BR/MX/CO + secular CL/AR + LGBTQ+ OK в большинстве LATAM, но осторожно с family-traditional segments), (7) high-inflation pricing freshness (AR 200%+ → ≤7 days, BR/MX/CL/CO/PE → ≤30 days) + PIX/SPEI payment integration, (8) WhatsApp Business как primary canal (vs Meta Reels secondary в LATAM) + AI Act Brazil 2026 prep (PL 2338/2023 pending) + AI DISCLOSURE bilingual (ES/PT + EN). Этот пресет — единая точка истины.

**КОГДА АКТИВИРУЕТСЯ LATAM_PRESET:**

Профиль клиента = CRISIS_EXPERT / HIGH_TICKET_PRO_SERVICES / REAL_ESTATE_EXPAT / INFOBIZ / LOCAL_SERVICE / EDTECH / ECOM_IMPULSE / B2B_SAAS, и любой из триггеров:
— Целевая аудитория физически проживает в Brazil / Mexico / Argentina / Chile / Colombia / Peru.
— Hook / Dialogue / EXACT STRING / overlay на португальском (BR) или испанском с регионом-маркером (MX/AR/CL/CO/PE).
— Soul ID = эксперт-LATAM-локалец (advogado BR / abogado MX-AR-CL-CO-PE / nutricionista / financial advisor / corretor de imóveis), оказывающий услугу resident audience.
— Гео-mix: эксперт в одной LATAM-стране, аудитория — в другой (MX → CO LATAM-кросс-border).
— Чек ≥ 2 500 BRL / 8 000 MXN / 400 000 ARS / 400 000 CLP / 1 800 000 COP / 1 500 PEN за услугу ИЛИ subscription ≥ 80 BRL / 200 MXN.
— Платформа = Meta / TikTok / Click-to-WhatsApp Ads / Mercado Libre Ads / Google Ads с гео-таргетом ≥ 1 LATAM-страна.
— Profile contains flag `vertical=latam_legal | latam_realestate | latam_infobiz | latam_local_service | latam_edtech | latam_high_ticket | latam_ecom | latam_fintech`.
— Sub-vertical = Brazilian crypto exchange / Mexican fitness app / Argentine SaaS / Chilean private banking / Colombian fintech / Peruvian D2C — любой LATAM-resident-targeting slot.

Если хотя бы один триггер — все 8 правил ниже **обязательны**. Этот sub-profile **накладывается** на правила базовой вертикали, не заменяет их. При конфликте применяется **stricter rule**.

**ПРАВИЛО 1 — Workspace + модель + packs + локация LATAM (anti-stereotype):**

— Workspace = Cinema Studio + Higgsfield Audio для cinematic flight'ов (São Paulo Faria Lima glass tower interior / Mexico City Polanco-Reforma corporate floor / Buenos Aires Recoleta café Belle Époque exterior / Santiago Vitacura skyline rooftop golden hour / Bogotá Zona G modern coworking / Lima Miraflores cliff malecón). Marketing Studio = ТОЛЬКО для talking-head expert (local Soul ID в кадре, peer-tone, MOF/BOF).
— Модель = Veo 3.1 (default для cinematic + любой архитектурной LATAM-локации — Faria Lima glass curtain wall / Reforma Torre BBVA геометрия / Recoleta Beaux-Arts ломаются на Kling). Seedance 2.0 = fallback для establishing shots без архитектурной геометрии в near-field.
— Packs = PORTRAIT CU + ANTI-AI-LOOK + LIGHT CONSISTENCY (warm tropical-amber для BR/CO/PE; cool-amber для MX/AR/CL — учёт latitude-tone) + REFLECTION (glass tower / café window / malecón ocean) + FABRIC / HAIR (если senior advisor Soul ID + cross-ref §21 EXECUTIVE-CALIBRATED HUMANIZATION) + SKIN-TEXTURE-LATAM (mixed-race casting realistic skin without over-tan / over-pale uniform — LATAM = mestizo / Afro-Latino / European-descendant / indigenous mix).
— Higgsfield Audio — major key, тёплый vocal-led; для BR допустим subtle bossa-jazz tempo (НЕ samba-stereotype / НЕ carnival-percussion). Для MX — modern Latin-pop tempo (НЕ mariachi / НЕ ranchera). Для AR — modern soft jazz (НЕ tango-stereotype). Для CL — minimalist neo-soul (НЕ cueca). Локализация voice-over: PT-BR neutral paulista accent для BR (НЕ carioca / НЕ nordestino без сегмент-обоснования); ES neutral по стране — limeño-neutral для PE, rioplatense для AR (с distinct «sh» для ll/y), chileno-neutral для CL, costeño/rolo для CO segment-specific, mexican-neutral DF-based для MX.
— **Anti-stereotype HARD BLOCK:** запрет в кадре Carnival costume / sombrero charro / mariachi guitar / tango couple на улице / Christ Redeemer лоб в лоб / Machu Picchu touristic frame / cactus desert / banana republic shot / favela poverty-porn / drug-cartel iconography / coca leaf prominently. Деталь locality — да, туристический штамп — нет.
— UI / маршрутный лист / PIX QR-code / fee schedule / документ-overlay в кадре = ВСЕГДА через §19A UI MOCKUP PIPELINE или §19B INFOGRAPHIC-CHART (special characters PT/ES — ã ç ñ á é í ó ú — Veo near-field рендерит с галлюцинациями).
— Format = 9:16 + 9s (Meta + IG Reels + TikTok primary) / 9:16 + 15s для Click-to-WhatsApp Ads (более длинная nurture-window) / 16:9 + 12-15s (YouTube + LinkedIn для HIGH_TICKET / B2B) / 1:1 + 7s (Mercado Libre / карусель листинг).

**ПРАВИЛО 2 — Language disambiguation (BR=Portuguese, остальные=Spanish с regional dialect):**

— **Brazil = Portuguese ONLY.** Hard-block: испанский voice-over / overlay / EXACT STRING для BR-аудитории. Brazilian audience читает испанский как чужой / colonialista «kombo-LATAM» подход. Если LATAM-flight общий и BR включён → отдельный PT-BR creative-set обязателен, не «spanglish».
— **Mexico = ES-MX neutral** (DF/CDMX accent baseline). Local slang («chido» / «padre» / «órale») — только если сегмент сильно youth + casual. Default — neutral, без слишком mexicano-coloquial для professional verticals.
— **Argentina = ES-AR rioplatense** — distinct «sh» для ll/y (calle = «cashe»), voseo («vos tenés» НЕ «tú tienes»), local terms («laburo» вместо «trabajo» для casual). Castilian-ES (Spain) tone в AR creative = звучит alien, читается как «европейский ad без локализации».
— **Chile = ES-CL chileno** — distinct prosody, «po» как enclitic («sí po», «no po»), местные термины («pololo» = boyfriend, «cachái» = casual «get it?»). НО для formal HIGH_TICKET / banking — neutral ES без chilenismos.
— **Colombia = ES-CO с региональным split** — costeño (Cartagena/Barranquilla, melodic) vs paisa (Medellín, «pues» enclitic) vs rolo (Bogotá, более formal neutral). Default для national reach = rolo neutral. Costeño / paisa = sub-segment specific.
— **Peru = ES-PE limeño** — neutral Andean-influenced ES, slower cadence чем mexicano. Quechua-loanwords только для специфических segments (Cusco / Arequipa cultural-themed).
— **Spanglish guardrail** — допустим ТОЛЬКО для ECOM_IMPULSE + youth audiences MX/PR cross-border. Для всех LATAM-resident professional verticals — clean ES без code-switching (выдаёт low-effort production + читается как «адаптация для US-Hispanic, не для local LATAM»).
— **Hard rule:** voice-over actor должен быть native того dialect (НЕ «universal Latin Spanish» от Madrid-studio) — иначе классифицируется как «foreign brand» и trust-score падает.

**ПРАВИЛО 3 — LGPD (BR) + LFPDPPP (MX) + Ley 25.326 (AR) + Ley 19.628/21.719 (CL) data protection compliance:**

**(a) Brazil — LGPD (Lei 13.709/2018) + ANPD enforcement:**
— LGPD Art. 7 — 10 legal bases (consent / contract / legal obligation / public policy / research / legitimate interest / credit protection / vital interest / health / regular exercise of rights). Targeting на «interest: Russian Orthodox»-аналог в BR (например, religion-affinity или political-affinity) = **Art. 11 special category** → explicit consent.
— **ANPD** (Autoridade Nacional de Proteção de Dados) — federal regulator, enforcement с 2021, fines до 2% revenue (max R$50M per infraction).
— **DPO обязателен** (Encarregado) — для public controllers и large-scale processing. Должен быть указан на лендинге.
— **Cross-border transfer outside BR** — ANPD adequacy decision (limited list) ИЛИ Standard Contractual Clauses ИЛИ binding corporate rules ИЛИ explicit consent. Meta/Google → US transfer требует SCC документации.
— Запреты в targeting Meta BR: **religion-affinity** (Catholic / Evangelical / Spiritist) → Art. 11; **health-condition affinity** → Art. 11; **political-affinity** (PT / PSOL / PL party-related) → Art. 11.
— Cross-ref Marco Civil Internet (Lei 12.965/2014) — net neutrality + privacy basics + judicial-order required для content takedown.

**(b) Mexico — LFPDPPP + INAI:**
— LFPDPPP (2010) — consent default required; aviso de privacidad обязателен на point-of-collection (лендинг, форма, WhatsApp opt-in).
— **INAI** — federal data authority, fines до 320 000 UMA (~$30M MXN). Cross-border transfer — privacy notice disclosure + recipient binding to similar protection level.
— Sensitive data (Art. 9) — racial origin, health, genetic, religious, political — explicit consent + special protection.

**(c) Argentina — Ley 25.326 + AAIP:**
— PDPA modeled на EU 95/46/EC pre-GDPR. Cross-border transfer = adequacy (EU listed adequate, но also adequacy для BR / IL / Switzerland).
— **AAIP** (Agencia de Acceso a la Información Pública) — enforcement. Fines от ARS 1 000 до ARS 100 000 (low ceiling, но reputational risk + class-action exposure).
— Pending update — Proyecto de Ley de Protección de Datos Personales (modernization, in legislature 2025-2026).

**(d) Chile — Ley 19.628 → Ley 21.719 (effective Dec 2026):**
— Текущий Ley 19.628 (1999) — outdated, weak enforcement.
— **Ley 21.719 effective December 2026** — major upgrade: создание **Agencia de Protección de Datos Personales**, fines до 20 000 UTM (~$1.4B CLP / ~$1.5M USD), GDPR-aligned (consent, DPO, DPIA, breach-notification 72h). LATAM-кампании, готовящиеся к запуску на 2026 → должны pre-comply сейчас.

**(e) Colombia (Ley 1581/2012 Habeas Data) + Peru (Ley 29733):**
— **CO SIC** (Superintendencia de Industria y Comercio) — enforcement Habeas Data. Cross-border = adequacy или contract. ANDI registry для databases.
— **PE ANPD** (Autoridad Nacional de Protección de Datos Personales) — enforcement Ley 29733 + Reglamento DS 003-2013-JUS.

**Запреты cross-LATAM (Meta detailed targeting):**
— Religious affinity / political party affinity / health-condition affinity / race-affinity / sexual-orientation — special-category в всех 6 юрисдикциях → hard-block detailed targeting; PASS — broad geo + behavior-based pixel events.
— Pixel sharing с третьими сторонами без DPA / DPIA → violations across LGPD Art. 39 / LFPDPPP / Ley 25.326.

PASS: broad country geo + interest «Cross-border tax / Real estate / Crypto investing / Fitness apps» + behavior-based pixel lookalike (1%) seeded from consented form-fill.
FAIL: «interest: Pentecostal Church + politcal: PT» AR-lookalike sourced from WhatsApp-group subscribers.

**ПРАВИЛО 4 — Consumer protection (CDC BR / PROFECO MX / SERNAC CL / Defensa del Consumidor AR / SIC CO / INDECOPI PE) + ANATEL/IFT subscription:**

— **Brazil — CDC (Lei 8.078/1990) + Procon (state-level):** abusive advertising forbidden (Art. 37 — misleading + abusive). Right of regret 7-day для distance contracts (Art. 49). Subscription auto-renewal — easy cancellation required + clear price-change notification. **ANATEL** — telecomm + ISP + subscription services regulator (premium SMS / IVR-based subscriptions hard-regulated).
— **Mexico — PROFECO + LFPC (Ley Federal de Protección al Consumidor):** publicidad engañosa prohibida. Subscription — easy cancel + auto-renewal disclosure. **IFT** — telecomm + broadcast + IPTV / OTT subscription regulator.
— **Argentina — Defensa del Consumidor (Ley 24.240):** boton de arrepentimiento (regret button) обязателен для onlne subscription (Resolution 424/2020) — visible cancel-link.
— **Chile — SERNAC + Ley 19.496:** право retracto 10 días для e-commerce + obligation на clear pricing + sin letra chica (no fine print) для material terms.
— **Colombia — SIC + Estatuto del Consumidor (Ley 1480/2011):** clear pricing + 5-day right of retraction online.
— **Peru — INDECOPI + Codigo de Protección y Defensa del Consumidor (Ley 29571):** publicidad veraz + clear cancellation для subscription.

**Subscription EdTech / SaaS / wellness universal LATAM rules:**
— Clear pricing с frequency (R$/mês, MXN/mes, ARS/mes) ВКЛЮЧАЯ tax (IVA / ICMS / IGV / ISS — country-specific).
— Easy cancel link / WhatsApp-cancel option visible на лендинге + recurring email reminder.
— Auto-renewal — pre-renewal notification ≥7 days (BR/MX/CL preferred, AR обязателен per Resolution 424).
— **Click-to-Cancel pattern** — FTC-аналог в LATAM = local consumer law equivalent; одинаковая complexity cancel ≤ sign-up.

**ПРАВИЛО 5 — Health / pharma / cosmetics (Anvisa BR / COFEPRIS MX / ANMAT AR / ISP CL / INVIMA CO / DIGEMID PE):**

— **Brazil — Anvisa (Agência Nacional de Vigilância Sanitária):** RDC 96/2008 для drug advertising + RDC 7/2015 для cosmetics. Supplements — registered as alimento с specific claim limits. **Запрет в крео:** «cura» / «trata» / «previne {disease}» для supplement / cosmetic — hard violation. PASS: «contribui para bem-estar» / «complemento alimentar» с anvisa registration # overlay.
— **Mexico — COFEPRIS (Comisión Federal para la Protección contra Riesgos Sanitarios):** NOM-072-SSA1 + Reglamento de Insumos para la Salud. Supplements — «suplemento alimenticio» NOT medicine. Cosmetics — notification COFEPRIS требуется. Запрет: therapeutic claim для non-drug.
— **Argentina — ANMAT (Administración Nacional de Medicamentos):** Resolución 215/2002 + 5350/2012 для drug advertising. Cosmetics — Disposición ANMAT 1109/99. Supplements — alimento с registration RNPA.
— **Chile — ISP (Instituto de Salud Pública):** Reglamento del Sistema Nacional de Control de Cosméticos + Decreto 466 для farmacéuticos.
— **Colombia — INVIMA:** Decreto 677/1995 для drug ads + Decreto 219/1998 для cosmetics.
— **Peru — DIGEMID (Dirección General de Medicamentos):** Ley 29459 + Reglamento.

**Disambiguation от US-FDA pattern:**
— US-FDA «structure/function claim» framework НЕ переносим в LATAM. Anvisa/COFEPRIS/ANMAT не имеют equivalent «structure/function exemption» — любой health-related claim требует local registration для конкретной категории.
— **Cross-LATAM запрет в крео:** «approved by FDA» — irrelevant в LATAM context, читается как «не зарегистрировано локально» = trust-score падает + regulator может расценить как substitution claim. PASS: указать местную registration «Reg. Anvisa #{N}» / «Reg. COFEPRIS #{N}» / «RNPA #{N}» / «Reg. INVIMA #{N}».
— **Influencer-endorsement health/cosmetic** — CONAR (BR self-regulation) + COFEPRIS + ANMAT — обязательно #publi / #publicidade / #publicidad disclosure + verifiable claim substantiation.

**ПРАВИЛО 6 — Financial (BACEN BR / CONDUSEF MX / CVM BR / CNBV MX / CMF CL / SFC CO / SBS PE) + crypto regional rules:**

**(a) Banking / fintech / payments:**
— **Brazil — BACEN (Banco Central do Brasil)** + **CVM (Comissão de Valores Mobiliários)** для securities. PIX — instant payment system mandatory для merchants (free для retail, low fee для B2B). Open Finance Brasil — regulated data-sharing framework.
— **Mexico — CNBV (Comisión Nacional Bancaria y de Valores)** + **CONDUSEF** (financial consumer protection) + **Banxico** (central bank). Ley Fintech (2018) — IFE (Instituciones de Fondos de Pago Electrónico) + ITF (Instituciones de Tecnología Financiera) registration. SPEI — instant payment system.
— **Chile — CMF (Comisión para el Mercado Financiero):** Ley Fintech 21.521 (effective 2024) — open finance + crowdfunding + crypto reporting.
— **Colombia — SFC (Superintendencia Financiera de Colombia)** + Sandbox la Arenera для fintech testing.
— **Peru — SBS (Superintendencia de Banca, Seguros y AFP)** + SMV для securities.
— **Argentina — BCRA + CNV** + crypto exchanges регулируются как PSAV (Proveedores de Servicios de Activos Virtuales) — registry CNV с 2024.

**(b) Crypto rules (regional split):**
— **Brazil — Lei 14.478/2022 (Marco Cripto) + Decreto 11.563/2023:** VASP registration требуется. Crypto exchange marketing — disclosure rules per CVM Resolution 175 + investor warnings.
— **Mexico — Ley Fintech:** только whitelisted crypto allowed через banks; restrictive framework.
— **Argentina — PSAV registry CNV 2024:** обязательная registration для exchange ops в AR jurisdiction.
— **Chile — Ley Fintech 21.521:** crypto under crowdfunding/PSAV regime.
— **Запрет в крео cross-LATAM:** «guaranteed returns» / «sin riesgo» / «100% safe» для crypto / investment — hard-block. PASS: «sujeto a riesgo de mercado · consulte prospecto · Reg. {regulator} #{N}».
— **CVM Resolution 175 (BR):** crowdfunding / tokenized securities — solicitation rules + accredited investor verification.

**Realistic example:**
— Brazilian crypto exchange = Marco Cripto VASP + CVM Resolution 175 + ANPD privacy + CONAR ad self-reg + PIX integration overlay.
— Chilean LATAM private banking = CMF Ley Fintech + AML Ley 19.913 + SERNAC consumer law + bilingual ES/EN overlay.

**ПРАВИЛО 7 — Currency / pricing localization (BRL / MXN / ARS / CLP / COP / PEN + inflation freshness):**

Валютная таблица:
— **Brazil → BRL (R$)** — основная. USD допустим ТОЛЬКО для US-tied service (FIRPTA / EB-5 / cross-border US-bank).
— **Mexico → MXN ($)** — основная (символ $ MX-context, не USD). USD допустим для premium services + border-region (Tijuana / Cd. Juárez) cross-border.
— **Argentina → ARS (peso)** — основная для local-resident services. **USD/USDT secondary** распространён в HIGH_TICKET / RealEstate (peso-volatility — sellers price в USD blue). Указание «al dólar MEP del día» или «al dólar oficial BCRA» обязательно.
— **Chile → CLP** — основная. UF (Unidad de Fomento) для real estate / mortgage / long-term contracts (inflation-indexed unit). USD только cross-border.
— **Colombia → COP** — основная. USD только для US-tied services.
— **Peru → PEN (sol)** — основная. USD допустим для real estate / premium services (USD pricing исторически принят в Lima upper-tier).

**Inflation freshness — per country:**
— **Argentina inflation 200%+ (2024-2025)** → pricing freshness **≤7 days**. Overlay: «precio actualizado al {fecha}» + «sujeto a actualización por inflación INDEC». Failing = credibility-killer + потенциально misleading per Defensa del Consumidor.
— **Brazil / Mexico / Chile / Colombia / Peru** → pricing freshness ≤30 days. Overlay: «preço válido até {data}» / «precio válido hasta {fecha}».
— **Chile UF anchor** — для long-term услуг → «valor en UF al {fecha}» снимает inflation-volatility вопрос полностью.

**Double-pricing rules:**
— **Cross-LATAM bundle** (MX + CO + PE single creative) — допустим если payment processing централизован (через Stripe LATAM или dLocal). Указать «MXN $ / COP $ / PEN S/» с актуальной FX rate ref «BCV / Banxico / DANE / BCRP».
— **Brazil — НЕ смешивать с другими LATAM** в одном creative (PT-only требование per ПРАВИЛО 2 + BRL-only валюта).

**Tax inclusion overlay:**
— BR — pricing с pode включать ICMS / ISS / Cofins (B2C — included; B2B — disclosed).
— MX — IVA 16% included or «más IVA» — must be explicit.
— AR — IVA 21% — same rule.
— CL — IVA 19% — same.
— CO — IVA 19% / régimen simple — disclosed.
— PE — IGV 18% — disclosed.

**ПРАВИЛО 8 — WhatsApp Business primary channel + AI DISCLOSURE bilingual + AI Act Brazil 2026 prep:**

**(a) WhatsApp Business / Click-to-WhatsApp Ads (доминирующий canal LATAM):**
— **WhatsApp penetration LATAM**: BR ~96%, MX ~93%, AR ~94%, CL ~91%, CO ~90%, PE ~88% (post-2024 stats). Это primary marketing canal vs Meta Reels secondary.
— **Click-to-WhatsApp Ads** через Meta — direct CTA «Mensaje en WhatsApp» / «Falar no WhatsApp». Conversion rate в 3-5x выше vs lead-form для LATAM.
— **24-hour customer service window** — после user-initiated message: free-form responses 24h. После — только pre-approved **template messages** (notifications, reminders, payment confirmation). Marketing templates требуют Meta approval + opt-in proof.
— **Opt-in обязателен** — explicit consent через лендинг / ad / form ПЕРЕД первым sending message. Без opt-in = WABA suspension risk.
— **Payment integration:** **PIX (BR)** native QR-code-on-WhatsApp via WhatsApp Payments BR (BACEN approved 2021); **SPEI (MX)** через third-party (Kueski / Mercado Pago). Указание payment method в крео = friction reduction.
— **Local platforms cross-ref:** Mercado Libre Ads для ecom; iFood (BR) / Rappi (MX/CO/AR/CL/PE) для delivery vertical — partnership-based ad placements ≠ Meta organic.

**(b) AI DISCLOSURE pack — bilingual обязательна для public LATAM-expert Soul ID:**
Если Soul ID = реальный лицензированный эксперт (OAB Brazil / Colegio de Abogados MX-AR-CL-CO-PE / CRM/CRP/CRF healthcare / CRECI BR real estate / AMPI MX) → AI DISCLOSURE pack ОБЯЗАТЕЛЕН.

Overlay-формат (bottom-third, present 0-end, ≤120 chars, Inter Regular 14pt, white-90% на gradient-darken):
— **BR:** `«Visual com IA · {Nome}, {Title} · {Registry} #{N} · {Firma}»` (PT-BR) + `«AI-assisted visual»` (EN secondary)
— **MX/AR/CL/CO/PE:** `«Imagen con IA · {Nombre}, {Título} · {Registro} #{N} · {Firma}»` (ES) + `«AI-assisted visual»` (EN secondary)
— Локализация — на государственном языке аудитории (PT для BR, ES для остальных) + EN дублирующая строка (международная читаемость для regulators + ad-library transparency).

**(c) AI Act Brazil prep (PL 2338/2023 — pending vote 2026):**
— Marco Legal da IA (PL 21/2020 + 2338/2023) — pending vote 2026. Если passed, Brazil → first-LATAM-AI-Act с proactive transparency obligation для AI-generated content depicting real persons.
— **Pre-comply now** — overlay в **(b)** покрывает ожидаемое требование. Sign-off на marketing ≠ sign-off на AI-generation — требуется отдельный AI-generation release.
— **CONAR self-reg (BR ad industry)** — уже требует AI-disclosure для influencer/expert content в digital ads (Guia CONAR 2024).

**(d) Religious / LGBTQ+ messaging guardrail:**
— **Religious sensitivity** — BR (Catholic ~50% + Evangelical ~30% growing) + MX (Catholic ~78%) + CO (Catholic ~73%) — religion-affinity targeting Art. 11 LGPD / Art. 9 LFPDPPP блокирован. Но messaging — НЕ rejecting religious framing (для healthcare / family services может быть органичным). Default: secular-neutral; opt-in segments — separate creative-set.
— **LGBTQ+ inclusivity OK** в большинстве LATAM (BR, MX-DF, AR, CL, CO-urban) для urban / younger audiences. Conservative для rural / Evangelical-heavy segments — split creative-set. Не использовать как «forced wokeness» (читается как foreign brand-imposition).
— **Family-traditional segments** (MX-Bajío / CO-Antioquia / BR-Sul rural) — default family-presented messaging (mom-dad-kids), opt-in для alternative-family framing.

**LATAM_PRESET — финальный чек-лист (8 проверок перед выдачей промта):**

| # | Проверка | PASS если | FAIL → |
|---|---|---|---|
| 1 | Cinema Studio / Marketing Studio (per use-case) + Veo 3.1 + PORTRAIT CU + ANTI-AI-LOOK + LIGHT CONSISTENCY (warm tropical-amber BR/CO/PE; cool-amber MX/AR/CL) + REFLECTION + SKIN-TEXTURE-LATAM (mixed-race realistic) + Higgsfield Audio (major key, country-specific tempo БЕЗ stereotype); локация = Faria Lima / Polanco-Reforma / Recoleta / Vitacura / Zona G / Miraflores БЕЗ Carnival / mariachi / tango / Christ Redeemer / favela штампа; UI / PIX QR через §19A/19B | Да | Переключить workspace; подключить packs; убрать stereotype; переключить локацию; переключить Audio tempo |
| 2 | Language disambiguation: BR = PT-BR ONLY (НЕ ES); MX = ES-MX neutral; AR = ES rioplatense (sh + voseo); CL = ES-CL (po enclitic для casual); CO = rolo neutral default; PE = limeño neutral; voice-over actor = native dialect; spanglish OFF для professional verticals | Да | Создать separate PT-BR creative для BR; перезаписать voice-over с native-dialect actor; убрать spanglish |
| 3 | Data protection: LGPD (BR ANPD + DPO + cross-border SCC) / LFPDPPP (MX INAI + aviso de privacidad) / Ley 25.326 (AR AAIP) / Ley 21.719 pre-compliance (CL 2026) / Habeas Data 1581 (CO SIC) / Ley 29733 (PE) — НЕТ religion/political/health affinity targeting; broad geo + behavior-based; explicit consent funnel для retargeting; pre-flight `meta-policy-checker` категория «LATAM — country DPL» | Да | Расширить targeting; отключить sensitive-affinity seed; добавить aviso de privacidad / DPO; SCC для cross-border |
| 4 | Consumer protection: CDC (BR Procon + ANATEL для subscription) / PROFECO + IFT (MX) / SERNAC (CL) / Defensa del Consumidor + Resolución 424 botón arrepentimiento (AR) / SIC (CO) / INDECOPI (PE); easy-cancel WhatsApp / link visible; pre-renewal notification ≥7 days; tax-inclusion overlay (IVA/ICMS/IGV/ISS) | Да | Добавить cancel-link; добавить pre-renewal notification; добавить tax-overlay; добавить ARS regret button |
| 5 | Health / cosmetic / supplement: НЕТ «cura/trata/previne {disease}» claim для non-drug; local registration overlay (Anvisa BR / COFEPRIS MX / ANMAT AR / ISP CL / INVIMA CO / DIGEMID PE) #{N}; НЕТ «FDA approved» substitution claim; influencer #publi / #publicidade / #publicidad disclosure (CONAR + local equivalent) | Да | Перепис на «contribui para bem-estar» / «complemento»; добавить local registration #; убрать FDA reference; добавить #publi |
| 6 | Financial / crypto: BACEN+CVM Marco Cripto (BR) / Ley Fintech CNBV+CONDUSEF (MX) / Ley Fintech 21.521 CMF (CL) / SFC (CO) / SBS+SMV (PE) / PSAV registry CNV (AR); НЕТ «guaranteed returns / sin riesgo / 100% safe»; «sujeto a riesgo de mercado · Reg. {regulator} #{N}» disclaimer; CVM Resolution 175 для tokenized / crowdfunding (BR) | Да | Заменить «guaranteed» на risk-disclosure; добавить regulator registration #; verify VASP/PSAV/IFE registration |
| 7 | Currency / pricing localization: BRL (BR) / MXN (MX) / ARS-USD (AR) / CLP-UF (CL long-term) / COP (CO) / PEN (PE); inflation freshness ≤7 days AR, ≤30 days other; tax-inclusion (IVA/ICMS/IGV/ISS); НЕ смешивать BR с другими LATAM в одном creative; USD только для US-tied service; cross-LATAM bundle ОК через Stripe LATAM / dLocal с FX ref | Да | Заменить валюту на основную per гео; добавить дату freshness AR; добавить tax-overlay; разделить BR creative |
| 8 | WhatsApp Business + AI DISCLOSURE pack + AI Act BR prep: Click-to-WhatsApp Ads CTA primary; opt-in form pre-message; template-message approved для outside-24h; PIX (BR) / SPEI (MX) integration shown; AI DISCLOSURE overlay bilingual (PT-BR/ES + EN) для real-expert Soul ID; separate AI-generation release документ; religion/LGBTQ+ split creative-sets per segment | Да | Добавить opt-in form; approve WhatsApp template; добавить AI DISCLOSURE bilingual; добавить отдельный AI-generation release; split religion-sensitive creative |

8/8 PASS → промт выпускается ученику.
≤7/8 PASS → return to editor, не выпускается. Конкретные FAIL-причины перечисляются в return-note.

**LATAM — 3-STEP FUNNEL GRANULARITY:**

| Step | Цель | Арка | EXACT STRING focus + Tone | CTA | Регуляторный риск |
|------|------|------|---|-----|---|
| TOF | Awareness — образовательный контент для cold LATAM-аудитории (problema-агностик) | F через §19B ANIMATED-DIAGRAM (3-этапа процедуры / market overview без identifiable people) | «Você» (BR formal) / «Tú» (MX/CO casual) / «Vos» (AR) / «Usted» (CL/CO formal) — peer authoritative · warm palette · neutral voice-over · НЕТ stereotype | «Baixe o guia» / «Descargá el checklist» / «Recebá WhatsApp-конспект» | Низкий (educational, не financial promo / не listing-specific) |
| MOF | Nurture — авторитет местного эксперта (методология + caso anônimo без NDA) | C — local-expert Soul ID tripod talking-head + §19B EDUCATIONAL-PROGRESSION (1→4 этап) | Peer empathic · adapted-LATAM-business wardrobe · WhatsApp-conversation tone | «Agende uma conversa de 15 min no WhatsApp» / «Bookeá un diagnóstico» / «Recibí methodology PDF» | Средний (substantiation OAB/Colegio claims + LGPD/LFPDPPP retargeting + Anvisa/COFEPRIS substantiation если health) |
| BOF | Close — confidential pre-qualified call, конкретный action на платную | C — эксперт Soul ID + signed engagement letter overlay + PIX/SPEI QR | Confident neutral · sin presión · конкретно | «Contrate la sesión paga {BRL/MXN/ARS/CLP/COP/PEN}» / «Comece com R$ {X}» | Высокий (OAB Brazil 34/1994 solicitation rules cold; FCA-LATAM-аналог = CVM/CNBV/CMF; sanctions screening для cross-border HIGH_TICKET) |

GUARDRAIL: Cold BOF CTA в open feed для регулируемых услуг (LEGAL / financial / health LATAM) — высокий риск OAB/CVM/COFEPRIS/CONAR violation. Default — soft TOF/MOF в cold; BOF — только в retargeting / WhatsApp warm после opt-in + identity verification.

CROSS-REFERENCES (не дублировать в этом пресете, использовать ссылками):

— **EU_RUSSIAN_DIASPORA_PRESET (волна Т.8)** — sub-profile pattern (8-rule structure, конфликт = stricter rule, накладывание на вертикаль) применён 1-to-1. Особо: AI DISCLOSURE pack bilingual pattern (ПРАВИЛО 8 здесь = ПРАВИЛО 8 EU_RD), GDPR Art.9 = LGPD Art.11 / LFPDPPP Art.9 / Ley 25.326 special-category — same logic, разные local регуляторы.
— **HIGH_TICKET_PRO_SERVICES_PRESET ПРАВИЛО 8** (AI DISCLOSURE для senior partner Soul ID) + **REAL_ESTATE_EXPAT_USA_PRESET ПРАВИЛО 8** (broker AI DISCLOSURE) — pattern переиспользуется в ПРАВИЛО 8 LATAM (OAB/CRECI/AMPI registry overlay bilingual PT-BR/ES + EN).
— **REAL_ESTATE_EXPAT_USA_PRESET ПРАВИЛО 6** (FIRPTA / SEC Reg D / TRID) — применима если LATAM-resident инвестирует в US-property через LATAM-broker; double-preset активация (US preset + LATAM sub-profile).
— **CRISIS-AUDIT-LAYER** — substantiation для LEGAL claim (processo OAB / sygnatura-аналог LATAM = N. CNJ Brazil / Expediente MX / Causa AR) применима для cross-border LATAM legal services из ПРАВИЛО 5/6.
— **KIDS_PARENTS_PRESET / KIDS_PARENTS_EDTECH_PRESET** — overlay при LATAM EDTECH; CHILDREN IN FRAME pack + COPPA-аналог (BR — ECA Estatuto da Criança + LGPD Art.14 minors; MX — LFPDPPP Art.36 minors) — cross-ref для bilingual minor-data inference.
— **Currency-таблица + MULTI-CURRENCY FOOTER rule** — расширение в ПРАВИЛО 7 для LATAM-specific (UF Chile + ARS-USD blue MEP + PIX/SPEI integration).
— **§11 Soul ID guardrails + §15 NEVER #23 meta-policy блокер + §19A UI MOCKUP + §19B INFOGRAPHIC-CHART + §21 EXECUTIVE-CALIBRATED HUMANIZATION** — универсальные guards, применимы поверх preset.
— **V17 (Жертва hook check) + V18 (substantiation) + V19-BIOCLAIM** — universal guards, проходятся поверх ПРАВИЛО 2 (визуал) + ПРАВИЛО 5 (health substantiation) + ПРАВИЛО 6 (financial claim).
— **ECOM_IMPULSE_USA_PRESET ПРАВИЛО 9 (AI DISCLOSURE)** — pattern, переиспользуется для LATAM ECOM (Mercado Libre listings + Click-to-WhatsApp).
...
═══════════════════════════════════════════════════
APAC_PRESET — sub-profile поверх вертикали для JP/KR/SG/AU (волна Т.15 после Т.15-А создателя)
═══════════════════════════════════════════════════

Контекст: «APAC mature markets — Japan / Korea / Singapore / Australia» — это **горизонтальный sub-profile**, который накладывается поверх вертикали (INFOBIZ / LOCAL_SERVICE / HIGH_TICKET_PRO_SERVICES / ECOM_IMPULSE / WELLNESS_HEALTH_RESTRICTED / EDTECH / REAL_ESTATE_EXPAT / SAAS / FINTECH), а не заменяет её. До волны Т.15 покрытие APAC было фрагментарным (US-centric defaults в большинстве paks, EU-centric privacy слой в EU_RUSSIAN_DIASPORA_PRESET, упоминания AU TGA в WELLNESS-аналоге, отсутствие японского inclusive-tax pricing). Для APAC агент терял восемь критических слоёв: (1) APPI / PIPA / PDPA / Privacy Act 1988 + национальные DPA enforcement matrix (PPC, PIPC, PDPC, OAIC), (2) cross-border data transfer ограничения per country (Korea blocked Google Maps export precedent, APPI standard-contract clauses 2022, PDPA whitelisted-jurisdiction model, Australian APP 8 accountability principle), (3) consumer protection layer (Tokutei Shōtorihiki Hō, KFTC, CCCS, ACCC) с DTC-ad nuances не покрытыми FTC ROSCA, (4) health / supplements регуляторика (Yakuji Hō, MFDS, HSA, TGA) с disambig от US-DSHEA structure-function approach, (5) financial / crypto matrix (FSA Japan, FSC Korea, MAS Singapore, ASIC Australia) с jurisdiction-specific licensing для fintech / crypto / private banking promo, (6) tax-inclusive pricing обязательность Japan (consumption tax 10%) + GST inclusive Singapore + GST Australia, (7) alternative channels LINE (JP) / KakaoTalk (KR) / WeChat (limited APAC) которые не покрыты Meta / TikTok / Instagram, (8) AI regulatory matrix (METI AI Guidelines 2024 JP, AI Basic Act 2024 KR effective 2026, AI Verify SG 2023, AI Safety Standard AU 2024) + bilingual AI DISCLOSURE на национальном языке + английском. Этот пресет — единая точка истины для APAC.

**КОГДА АКТИВИРУЕТСЯ APAC_PRESET:**

Профиль клиента = INFOBIZ / LOCAL_SERVICE / HIGH_TICKET_PRO_SERVICES / ECOM_IMPULSE / WELLNESS_HEALTH_RESTRICTED / EDTECH / REAL_ESTATE_EXPAT / SAAS / FINTECH, и любой из триггеров:
— Гео-таргет включает JP / KR / SG / TW / HK / AU (а также NZ как Australia-overlay).
— Hook / Dialogue / EXACT STRING / overlay на японском (日本語) / корейском (한국어) / Mandarin (中文 traditional для TW/HK, simplified для SG) / SG-English / AU-English.
— Soul ID = APAC-локальный эксперт (japanese skincare founder / Korean K-pop SaaS CEO / Singapore private banker / Australian fitness chain owner).
— Чек ≥ ¥10 000 / ₩100 000 / S$100 / A$150 за услугу или ≥ ¥3 000 / ₩30 000 / S$30 / A$50 за consumable subscription.
— Sub-vertical = japanese anti-aging beauty / Korean cosmetic surgery referral / Singapore private banking onboarding / Australian fitness chain membership / TW e-commerce / HK insurance broker / KR cosmetic dermatology.
— Profile contains flag `vertical=apac_jp_*|apac_kr_*|apac_sg_*|apac_au_*|apac_tw_*|apac_hk_*`.
— Платформа = Meta / TikTok / Instagram / YouTube + LINE Ads (JP/TW) / KakaoMoment (KR) / Naver Search Ads (KR) / WeChat Ads (если SG-Mandarin или HK).

Если хотя бы один триггер — все 8 правил ниже **обязательны**. Этот sub-profile **накладывается** на правила базовой вертикали (WELLNESS_HEALTH_RESTRICTED / ECOM_IMPULSE_USA / HIGH_TICKET_PRO_SERVICES_PRESET), не заменяет их. При конфликте применяется **stricter rule** (например, Yakuji Hō JP запрет на before/after для cosmetics строже чем DSHEA structure-function — применяется Yakuji Hō).

**ПРАВИЛО 1 — Workspace + локация APAC (mature urban CBD, не stereotype reduction):**

— Workspace = Cinema Studio + Higgsfield Audio для cinematic beauty / luxury / HIGH_TICKET / fintech (Tokyo Marunouchi glass-tower lobby golden hour / Shibuya Sky observation deck blue hour / Seoul Gangnam Teheran-ro office tower / Singapore Marina Bay financial district / Sydney CBD Barangaroo / Melbourne Docklands). Marketing Studio = для UGC talking-head (Japanese skincare founder peer-tone, Korean K-pop SaaS founder, Singapore wealth advisor, Australian fitness coach).
— Модель = **Veo 3.1** (default для cinematic + architectural APAC — Tokyo / Singapore Marina Bay / Sydney Opera House / Seoul N-Tower геометрия ломается на Kling в near-field). **Seedance 2.0** = fallback для smooth lifestyle motion (matcha pouring, K-beauty serum application, Singapore hawker center plating, AU coastal jogging). **Kling 3.0** = только product macro (Japanese skincare bottle / Korean essence dropper / Singapore card tap / Australian protein scoop).
— Packs = PORTRAIT CU + ANTI-AI-LOOK + LIGHT CONSISTENCY (cool-neutral key для JP/KR luxury beauty; warm coastal для AU; mixed cool+warm для SG tropical) + REFLECTION (glass tower / marina water / shopping mall mirror) + FABRIC (если luxury Soul ID) + HAIR (Asian hair texture требует ANTI-AI-LOOK особенно жёстко — AI defaults дают «shampoo-commercial perfect» что читается как fake).
— Higgsfield Audio — major key vocal-led для JP/SG/AU; для KR допустим minor-major mix (K-pop influence легитимизирует). **Запрет** cultural-loaded music: enka old-school (JP — читается как boomer-targeting, проседает на 25-45), trot (KR — same), традиционная gagaku / gugak / gamelan / didgeridoo как primary trick (stereotype reduction). PASS: contemporary city-pop (JP), K-indie / lo-fi (KR), tropical house ambient (SG), AU indie folk / electronic.
— UI / pricing / product label / financial disclaimer overlay в кадре = ВСЕГДА через §19A UI MOCKUP PIPELINE или §19B INFOGRAPHIC-CHART, не напрямую в Veo / Kling (CJK глифы — kanji 漢字 / hiragana ひらがな / katakana カタカナ / hangul 한글 / Chinese 中文 — рендерятся с галлюцинациями глифов даже хуже чем Cyrillic, especially в near-field).
— **Локация запрет stereotype reduction:** geisha / kimono в primary frame для не-traditional product / sushi conveyor belt / Mount Fuji background для urban product / hanbok для не-traditional KR / kangaroo + Uluru background для не-tourism AU / merlion для не-tourism SG. **Допустимо** в outro brand frame если product = traditional category (matcha brand / hanbok rental / Australian wildlife tour / Singapore heritage tour).
— Format = 9:16 + 9s (Meta + IG Reels + TikTok + LINE VOOM primary для JP/SG/AU diaspora retail) / 9:16 + 15s (KR YouTube Shorts + KakaoTV) / 16:9 + 15s (YouTube + LinkedIn для HIGH_TICKET institutional) / 1:1 + 7s (карусель листинг).

**ПРАВИЛО 2 — Language disambiguation + cultural register (formal vs casual per country + JP age-gating):**

— **Japan (日本語)** = по умолчанию keigo (敬語 honorific) для аудитории 35+, нейтральный desu/masu (です・ます) для 25-45, casual da/de aru (だ・である) только для 20-30 lifestyle / fashion. **Запрет** mixed-register в одном крео. Voice-over: stage-Japanese (neutral Tokyo dialect), без Kansai-ben / Tohoku-ben (читается как comedy-relief, ломает premium / professional positioning). **Гендер**: female voice-over по умолчанию для beauty / wellness (cultural norm), male для finance / car / insurance.
— **Korea (한국어)** = jondaetmal (존댓말 honorific -습니다/-입니다) для официальных контекстов (finance / legal / medical), banmal-light (반말 -아/어요) для lifestyle / K-beauty / EDTECH. Возрастной register: 30+ — formal; 20-30 — casual-friendly. **Запрет** mixing honorific levels в одном крео.
— **Singapore** = SG-English (близок к UK-English с loanwords lah/leh/sia только в casual UGC, **запрет** в premium / B2B / financial) + Mandarin (упрощённое для SG-Chinese аудитории, traditional для TW/HK overlay). **Trilingual market reality** — SG крео часто требует EN-primary + Mandarin-subtitle overlay для Chinese-Singaporean segment. Malay / Tamil overlay для inclusivity flagged enterprise clients.
— **Australia (AU-English)** = AU register не identical UK/US — avoid «mate» как primary hook для не-blue-collar product (читается как stereotype), allowable для tradies / outdoor / sport / beer. Indigenous land acknowledgement — **обязательна** для real-estate / property / development / lifestyle premium брендов в TOF (соответствует Reconciliation Action Plan industry standard 2024+). Default overlay: «We acknowledge the Traditional Owners of the land where {brand} operates · {Traditional Owners name if known} · paying respects to Elders past and present».
— **Cultural register по сегменту:**
  • JP elder (50+) audience → keigo обязательно, никакого peer-tone, no first-name address.
  • KR elder (50+) audience → jondaetmal + indirect framing («귀하» / «고객님» вместо «당신»).
  • SG mature audience → SG-English neutral, no localism (lah/leh/sia OFF).
  • AU mature audience → AU-English neutral, no «mate / bloody / heaps» в premium positioning.
— **Voice-over локализация:** Native speaker обязателен (Synthesia / ElevenLabs synthetic voice для CJK ломается на тональность). Если AI voice-over — overlay «AI-generated narration · 音声はAI合成です / AI 음성 / AI-generated voice» (тоже под AI DISCLOSURE pack — см. ПРАВИЛО 8).
— **CJK text rendering** через §19A UI MOCKUP обязателен — kanji / hangul / Chinese glyphs в Veo / Kling near-field = галлюцинации (особенно сложные kanji 漢字, double-byte hangul, traditional Chinese).

**ПРАВИЛО 3 — APPI (JP) + PIPA (KR) + PDPA (SG) + Privacy Act 1988 (AU) data protection compliance:**

**(a) JP — APPI + PPC:** covers 個人情報 + 個人関連情報 (2020 amendment); cross-border transfer = explicit consent / adequacy decision (EU yes, US no) / PPC standardized contract 2022 / corporate binding rules. Sensitive category (medical / political / religion / criminal record / social status) = APPI Art. 17(2) strict opt-in. Запреты Meta: Custom Audience seeded из «Japanese-speaker outside JP» / «interest 日本語» = nationality-proxy → требует Art. 17 explicit consent; Cookie pixel без banner = Art. 27 violation. PASS: broad JP geo + behavior interests + cookie consent banner + privacy notice with PPC contact.

**(b) KR — PIPA + PIPC (strictest APAC):** 2011 + 2023 amendments, criminal liability 3 years + KRW 50M fines + admin до 3% revenue. PIPC enforcement: Meta KRW 6.4B 2024, Google KRW 100B+. Cross-border transfer = explicit consent + per-purpose notification (Korea blocked Google Maps export 2023 — strict data localization для sensitive data). Personal Information Safety Act + KISA encryption + access logging. KCC (telecomm SMS/push). Любой targeting requires Korean-language privacy notice + PIPC contact + retention period + cross-border recipient list. Sensitive opt-in **на корейском**. PASS: KR-resident IP geo + behavior interests + Korean-language privacy notice + KakaoMoment если primary.

**(c) SG — PDPA + PDPC + IMDA:** 2012 + 2020 amendments (mandatory breach notification + enhanced consent); admin до S$1M или 10% turnover (2022). Cross-border = adequate-protection comparable to PDPA (whitelisted-jurisdiction + contractual clauses + ASEAN MCC 2021). DNC Registry pre-screen для SMS / voice / fax. PASS: SG-resident geo + PDPA consent banner + DNC pre-screen + privacy notice with PDPC contact.

**(d) AU — Privacy Act 1988 + OAIC + APP:** Notifiable Data Breaches mandatory since 2018 (APP entities >A$3M turnover + health providers any turnover). OAIC admin до A$50M или 30% turnover (2022). APP 8 (cross-border accountability), APP 7 (direct marketing opt-out mandatory link), APP 11 (security). Privacy Act reform 2024-2025 closer to GDPR (consent definitions, statutory tort 2025). PASS: AU-resident geo + APP consent + opt-out link в каждом direct-marketing ads + privacy notice with OAIC contact.

Cross-ref CRISIS-AUDIT-LAYER C4 (combined-identifier risk).

**ПРАВИЛО 4 — Tokutei Shōtorihiki Hō (JP) + KFTC (KR) + CCCS (SG) + ACCC (AU) consumer protection layer:**

DTC consumer protection — наслаивается на base FTC ROSCA / Click-to-Cancel из ECOM_IMPULSE_USA_PRESET, но **stricter в APAC**.

**(a) Japan — Tokutei Shōtorihiki Hō (Specified Commercial Transactions Act / 特定商取引法):**
— DTC subscription requires «特定商取引法に基づく表記» (Specified Commercial Transactions disclosure) на лендинге: seller name, address, phone, total price (tax inclusive!), additional fees, delivery time, payment methods, returns / cancellation policy, expiry of offer.
— Cooling-off period 8 days для door-to-door / phone sales (не для online); НО online subscription auto-renew требует **clear consent confirmation page** до billing (similar to FTC ROSCA but stricter typography rules per METI guidance 2022).
— **Запреты:** «今だけ» (limited time) / «残り 3 個» (only 3 left) без real-stock substantiation = JFTC (Japan Fair Trade Commission) деceptive labeling enforcement. Replace: real-stock-based countdown.
— Tax inclusive pricing (см. ПРАВИЛО 7) — обязательно даже в крео-overlay для price display.

**(b) Korea — KFTC (Korea Fair Trade Commission) + Electronic Commerce Consumer Protection Act:**
— KFTC enforcement против deceptive ad — стрицер чем FTC §5 (administrative до 2% revenue + corrective ad order — competitor может потребовать публикации correction в same media).
— **«과장 광고» (exaggerated ad) enforcement** — даже soft claims «최고» (best) / «국내 1위» (#1 in Korea) требуют substantiation с timestamp.
— Cooling-off period — 7 days для online purchases (Electronic Commerce Consumer Protection Act).
— Subscription auto-renew — KFTC consent rules + Korean-language disclosure обязательны.

**(c) Singapore — CCCS (Competition and Consumer Commission of Singapore) + Consumer Protection (Fair Trading) Act:**
— CPFTA prohibits unfair practices — drip pricing, fake reviews, hidden subscription terms, false scarcity.
— CCCS guidelines for online endorsement (2020 update) — sponsored content disclosure mandatory («#ad / #sponsored / Paid partnership»).
— **«Sale» / «Discount» enforcement** — original price должна быть genuine recent price (28-day reference rule similar to UK).
— Subscription terms — PDPA consent + CPFTA disclosure layer.

**(d) Australia — ACCC (Australian Competition and Consumer Commission) + Australian Consumer Law (ACL):**
— ACL Schedule 2 Competition and Consumer Act 2010 — misleading or deceptive conduct strict liability.
— **«Was/Now» pricing rule** — original price genuine + 28-day window before sale; ACCC penalties до A$50M.
— Subscription auto-renew — ACCC guidance + ASIC overlap если financial; «easy cancel» mandate consultation 2024.
— Influencer disclosure — AANA Code + ACCC enforcement (2024 sweep — $10M+ fines).

PASS пример (JP): `Overlay: "今だけ ¥3 980 (税込) · 通常価格 ¥5 980 · 在庫数限定" + 特商法表記 link in caption` (tax inclusive, anchor price prominent, real stock).
FAIL пример (KR): `Overlay: "최고의 한국 화장품 · 50% 할인 · 한정 수량!"` (unsubstantiated «최고» + percentage without anchor + artificial scarcity → KFTC + Korean ARL violation).

**ПРАВИЛО 5 — Health / supplements (Yakuji Hō JP / MFDS KR / HSA SG / TGA AU) — disambig от US-DSHEA approach:**

US-DSHEA structure-function approach (WELLNESS_HEALTH_RESTRICTED_USA_PRESET) **НЕ применяется напрямую в APAC**.

**(a) JP — Yakuji Hō (薬機法 / Pharmaceutical Affairs Act, renamed 2014):** запрет efficacy claims за пределами approved 機能性表示食品 / トクホ. Cosmetics 化粧品 — claims ограничены 56 approved expressions (cleansing / moisturizing / sun protection). Запрет anti-aging «wrinkle removal» / «skin whitening» — только Quasi-Drugs 医薬部外品 с MHLW approval (limited 11 wrinkle / 9 whitening active ingredients 2017+). **Запрет before/after** для cosmetics — Yakuji Hō §66 + JAA standard, strict enforcement after 2023 PMDA crackdown. «医師推奨» — только verified medical license + consultation disclosure.

**(b) KR — MFDS (식품의약품안전처):** Health Functional Foods Act 건강기능식품법 — only approved functional ingredients (lutein, ginseng saponins, vitamin D) могут заявлять health claims. Functional cosmetics 기능성화장품 — MFDS pre-approval для anti-wrinkle / whitening / sunscreen. Cosmetic surgery referral — before/after = patient written consent + risk overlay; «no risk» / «100% success» = hard-block. «의사 추천» — Medical Service Act §56 prohibits direct medical advertising non-medical entities, referral agencies должны disclaim.

**(c) SG — HSA (Health Sciences Authority):** Health Products Act 2007 — «Health Supplements» exempt registration НО ad compliant HSA Guidelines. Запрет disease claims (treatment / prevention / cure) для non-registered. Cosmetics — ASEAN Cosmetic Directive + PIF safety dossier substantiation. «Doctor endorsed» — MOH + SMC ethical code.

**(d) AU — TGA + AHPRA:** Therapeutic Goods Act 1989 + TGA Advertising Code 2021/2024 — strict для AUST L / AUST R. Prohibited representations (cancer / mental illness / serious conditions) — hard-block без TGA pre-approval. Restricted representations (diabetes / arthritis) — TGA pre-approval. Запрет testimonials для prescription, restricted для OTC. AHPRA: no testimonials clinical care, no comparative claims, registration number mandatory. Запрет cosmetic surgery before/after без TGA + AHPRA compliance (2023 reform — no «easy / safe / risk-free» framing).

PASS overlay-формат (per country, на государственном языке + English):
— JP: «本品は機能性表示食品です。疾病の診断、治療、予防を目的としたものではありません · This is Food with Function Claims · Not for disease diagnosis, treatment, or prevention»
— KR: «본 제품은 건강기능식품으로서 질병의 예방 및 치료를 위한 의약품이 아닙니다 · This is a Health Functional Food · Not a medicine for prevention or treatment of disease»
— SG: «This product is a health supplement. It is not intended for prevention, treatment, or cure of any medical condition. Consult healthcare professional if pregnant or have medical conditions.»
— AU: «Always read the label and follow the directions for use. If symptoms persist, consult your healthcare professional. AUST L {number}.»

**ПРАВИЛО 6 — Financial (FSA JP / FSC KR / MAS SG / ASIC AU) + crypto APAC matrix:**

**(a) JP — FSA + JFSA:** FIEA (金融商品取引法) — investment / securities promotion requires Type I/II Financial Instruments Business registration. Crypto-Asset Exchange (暗号資産交換業) — JFSA registration + JVCEA self-regulatory compliance. **«元本保証» prohibition** для unregistered — hard-block. FSA 2024 guidance on social media influencers — sponsored disclosure obligatory.

**(b) KR — FSC + FSS:** Capital Markets Act + Insurance Business Act — strict licensing + disclosure. Crypto asset advertising — Virtual Asset User Protection Act (effective July 2024) + FSC enforcement (KakaoTalk / Naver restrictions для unregistered). «보장» / «수익률» claims — substantiation + risk disclosure (FSS administrative orders).

**(c) SG — MAS:** Securities and Futures Act + Financial Advisers Act — licensed advice only; «general advice» disclaimer для unlicensed. Payment Services Act 2019 + 2024 amendments: **MAS prohibits crypto retail ads в public spaces / social media targeting SG consumers** since January 2022 (только corporate channels + licensed VASP). Private banking — MAS Notice FAA-N16 (CKA accredited/qualified investor).

**(d) AU — ASIC:** Corporations Act 2001 — AFSL required для financial product advice / dealing. «No advice» disclaimer для general info. Crypto — ASIC INFO 225 (2022 classification) + ACCC scams enforcement. Design and Distribution Obligations (DDO, 2021) — target market determination obligatory.

**APAC crypto matrix summary:**
| Country | Status | Retail Ad |
|---|---|---|
| JP | Licensed (JFSA + JVCEA) | Allowed with disclosure |
| KR | Licensed (FSC + VAUPA 2024) | Restricted (no retail aggressive ad) |
| SG | Licensed (MAS PSA) | **Hard-block для retail ads** |
| AU | Mixed (ASIC info sheet) | Allowed with AFSL + DDO |

PASS пример (SG fintech): `Overlay: "MAS-licensed payment institution · Licence No. PS20200001 · Capital protected up to S$75,000 by SDIC for eligible deposits"`.
FAIL пример (SG crypto): `Hook: "Buy Bitcoin instantly · 0% fees · Singapore residents welcome"` — hard-block MAS retail crypto ad rules.

**ПРАВИЛО 7 — Currency / pricing localization + tax inclusive pricing (JP obligatory):**

Валютная таблица + tax rules:
— **JP** → **JPY (¥)** основная; **JPY все цифры значимые** (нет «1.234,56» формата — пишется «1,234円» или «1234円»); **tax inclusive (税込) обязательно** per Japan Consumer Protection Law amendment 2021 (consumption tax 10% / reduced 8% for food). Default: «¥3,980 (税込)» НЕ «¥3,620 (税抜)» without inclusive equivalent.
— **KR** → **KRW (₩)** основная; **VAT 10% включается** в displayed price (Korean Value-Added Tax Act); большие числа — «₩100,000» формат.
— **SG** → **SGD (S$)** основная; **GST 9% (2024)** — для consumer-facing ads recommended GST-inclusive display (CPFTA fair pricing); B2B допускает exclusive с GST overlay.
— **AU** → **AUD (A$)** основная; **GST 10%** — Trade Practices Act mandates GST-inclusive для consumer ads (ACCC enforcement strict).
— **TW** → **TWD (NT$)** основная; **VAT 5%** включается.
— **HK** → **HKD (HK$)** основная; no GST (free port).
— **USD в APAC creо** — default OFF; допустим ТОЛЬКО если product / service explicitly USD-denominated (e.g., US-real-estate cross-ref, international remittance, US-equity investment). В этом случае dual-pricing с national currency обязателен.

**Tax inclusive enforcement:**
— **JP**: «¥3,980 (税込)» обязательно — без 税込 = JFTC enforcement / consumer protection violation. Если в Veo / Kling overlay пишется без 税込 — fail на §19A UI MOCKUP step.
— **AU**: «$49.95 incl. GST» или просто «$49.95» (с подразумеваемым GST inclusive в consumer context) — exclusive pricing требует жирный «+ GST» overlay.
— **SG**: «S$49.95 incl. GST» рекомендуется для consumer; B2B может «S$49.95 + GST».
— **KR**: VAT inclusive default — no special overlay.

**Double-pricing rules:**
— Допустимо ТОЛЬКО при cross-border legit kontext (SG broker для AU-resident → S$ + A$; JP brand cross-selling в KR → ¥ + ₩). Не дублировать в single-market creо.
— Расхождение ±3% от ECB / cross-rate → substantiation overlay «exchange rate as of {date}».

**Pricing prominence:**
— typical price prominence equal с «from / 從 / から / 부터» — иначе ACCC / KFTC / CCCS misleading pricing violation. «From ¥1,000» БЕЗ typical → «typical ¥3,980 / starter ¥1,000».

**ПРАВИЛО 8 — LINE / KakaoTalk alternative channels + AI DISCLOSURE bilingual + AI Basic Act 2024 (KR) / METI AI Guidelines (JP) / AI Safety Standard (AU):**

**(a) Alternative channels coverage:**
— **JP**: **LINE Ads Platform (LAP)** обязателен в media-mix для 25-55 (LINE penetration 95%+ adults). Format = 9:16 + 6-15s LINE VOOM, LINE Talk Head View, LINE Smart Channel. LINE-specific moderation: no political / religious / dating / gambling без verified approval.
— **KR**: **KakaoMoment Ads + Naver Search Ads + Naver Performance Display Ads** обязательны (KakaoTalk 90%+ adults, Naver 75% search share). Format = 9:16 + 9-15s KakaoMoment, 1:1 для Naver smart channel. KakaoMoment moderation: PIPA-compliant data, no health-disease claims (MFDS).
— **SG**: Meta / TikTok / Instagram primary + WeChat Ads если Mandarin SG-Chinese segment (limited). LINE has TW/JP overlap для travel / luxury cross-promo.
— **AU**: Meta / TikTok / Instagram primary; LinkedIn для HIGH_TICKET. No major local alternative — Australian audience uses global platforms.
— **WeChat APAC** — limited to SG-Chinese / HK / TW diaspora — but mainland China targeting requires WeChat Official Account + Tencent Ads license (out of scope APAC_PRESET — отдельный CN preset future wave).

**(b) AI DISCLOSURE pack — обязательна для AI-generated Soul ID / voice / product visualization:**

Если Soul ID = реальный лицензированный профессионал (Bar, Medical Council, MAS-licensed FA, AHPRA, MFDS-approved clinician) → AI DISCLOSURE pack ОБЯЗАТЕЛЕН. Pattern совпадает с HIGH_TICKET_PRO_SERVICES_PRESET ПРАВИЛО 8 + EU_RUSSIAN_DIASPORA_PRESET ПРАВИЛО 8.

Overlay-формат (bilingual: национальный язык + English, bottom-third, present 0-end, ≤120 chars per line, Inter Regular 14pt, white-90% на gradient-darken):
— JP: «AI生成映像 · {名前} {肩書} · {登録番号} · AI-generated visual · {Name}, {Title} · Reg #{N}»
— KR: «AI 생성 이미지 · {이름} {직책} · {등록번호} · AI-generated visual · {Name}, {Title} · Reg #{N}»
— SG: «AI-generated visual · {Name}, {Title} · MAS/HSA/SMC Reg #{N}» (English primary, Mandarin overlay если CN-target segment)
— AU: «AI-generated visual · {Name}, {Title} · AHPRA/ASIC/AFSL #{N}»

**(c) Country AI regulatory matrix:**
— **JP**: **METI «AI Guidelines for Business» (2024)** + **AI Promotion Bill (draft 2026)** + **Copyright Act AI training amendments 2024**. Soft-law approach + transparency for AI-generated content recommended (PPC guidance 2024).
— **KR**: **AI Basic Act 2024 (passed, effective January 2026)** — high-risk AI requires impact assessment + user notification + Korean-language AI-generated content disclosure. Strictest APAC AI law.
— **SG**: **AI Verify framework (IMDA 2023)** + **Model AI Governance Framework (2nd ed. 2020 + Generative AI extension 2024)** — voluntary disclosure recommended, mandatory для public-sector procurement.
— **AU**: **AI Safety Standard (DISR 2024) — Voluntary AI Safety Standard 10 guardrails** + **proposed mandatory guardrails for high-risk AI (2025)**. ACCC + OAIC enforcement on AI-generated misleading content.

**(d) Cross-border AI obligation:**
Если AI-generated likeness используется в крео targeting KR-resident → AI Basic Act 2026 extraterritorial obligation (similar to EU AI Act art.50) — KR-language AI-generation disclosure обязательна independently от producer location. JP / SG / AU — voluntary но highly recommended для brand-safety.

**APAC_PRESET — финальный чек-лист (8 проверок перед выдачей промта):**

| # | Проверка | PASS если | FAIL → |
|---|---|---|---|
| 1 | Cinema Studio / Marketing Studio (per use-case) + Veo 3.1 / Seedance 2.0 / Kling 3.0 (per use-case) + PORTRAIT CU + ANTI-AI-LOOK (особенно HAIR pack для Asian hair) + LIGHT CONSISTENCY (cool-neutral JP/KR, warm AU, mixed SG) + REFLECTION + FABRIC; Higgsfield Audio major key (KR допускает minor-major mix); CJK через §19A; формат per platform (9:16 LINE/KakaoMoment/Reels); локация = mature urban CBD (Tokyo Marunouchi / Seoul Gangnam / Singapore Marina Bay / Sydney CBD) без stereotype reduction (geisha / kimono / kangaroo / merlion) | Да | Переключить workspace; подключить недостающие packs; убрать stereotype prop; переключить локацию на urban-CBD |
| 2 | Language disambiguation per country: JP register keigo (35+) / desu-masu (25-45); KR jondaetmal (formal) / banmal-light (lifestyle); SG-English (no lah/leh в premium); AU-English (no «mate» в premium, Indigenous land acknowledgement для real-estate / lifestyle premium); native voice-over (AI synthetic OFF без disclosure); CJK через §19A UI MOCKUP | Да | Переписать register под аудиторию; убрать localism из premium; native voice-over найти; AI voice-over overlay добавить |
| 3 | Privacy compliance per country: APPI (JP) + ASP cross-border consent / standardized contract; PIPA (KR) + PIPC Korean-language privacy notice + sensitive opt-in; PDPA (SG) + PDPC consent banner + DNC pre-screen; Privacy Act 1988 (AU) + APP 7 opt-out mechanism + OAIC contact; broad geo + behavior-based interests (НЕ language/nationality proxy); cookie consent banner | Да | Расширить targeting на broad geo; добавить country-specific privacy notice; убрать nationality/language seed; добавить DNC pre-screen; добавить opt-out link |
| 4 | Consumer protection per country: Tokutei Shōtorihiki Hō (JP) 特商法表記 link + tax inclusive + real-stock; KFTC (KR) substantiation для «최고/1위» + cooling-off; CCCS (SG) genuine-anchor + sponsored disclosure; ACCC (AU) was/now 28-day rule + influencer disclosure (AANA Code); subscription auto-renew per country rules | Да | Добавить 特商法表記 link; substantiate ranking claim; удалить artificial scarcity; добавить «#ad / Paid partnership» disclosure; correct «was/now» к 28-day rule |
| 5 | Health/supplements per country: Yakuji Hō (JP) cosmetics 56-expression limit + no before/after + 機能性表示食品 disclaimer; MFDS (KR) 건강기능식품 disclaimer + no «doctor recommendation» без medical license; HSA (SG) no disease claims + ASEAN Cosmetic Directive; TGA + AHPRA (AU) prohibited representations check + AUST L/R disclosure + AHPRA registration number; NO US-DSHEA structure-function template applied | Да | Удалить prohibited efficacy claim; добавить country-specific disclaimer overlay; substitute approved expressions; убрать before/after если Yakuji Hō / cosmetic surgery без consent; добавить registration number |
| 6 | Financial/crypto per country: FSA (JP) Type I/II registration overlay + no «元本保証»; FSC (KR) Capital Markets Act licensing + 보장/수익률 substantiation; **MAS (SG) hard-block для retail crypto ads** + licence disclosure; ASIC (AU) AFSL + DDO target market + no-advice disclaimer; crypto matrix per country verified | Да | Удалить retail crypto ad для SG; добавить licence overlay; substantiate yield claims; добавить DDO target market statement; switch на B2B-only если retail запрещён |
| 7 | Currency / tax-inclusive pricing: JPY ¥ tax inclusive (税込) обязательно; KRW ₩ VAT inclusive; SGD S$ GST inclusive (consumer); AUD A$ GST inclusive (ACCC); TWD NT$ VAT inclusive; double-pricing только при legit cross-border (substantiation by exchange rate); typical price prominence equal с «from / から / 부터»; USD OFF кроме US-denominated services | Да | Добавить (税込) к JP price; перевести в local currency; добавить GST inclusive disclosure AU/SG; добавить typical price prominence; substantiate exchange rate если cross-border |
| 8 | LINE/KakaoMoment/Naver Ads coverage per country (JP/KR primary alternative channels); AI DISCLOSURE bilingual overlay (CJK + English) для AI Soul ID / voice / product; AI regulatory matrix: METI AI Guidelines (JP), AI Basic Act 2024 (KR effective 2026) Korean-language disclosure, AI Verify (SG voluntary), Voluntary AI Safety Standard (AU); sign-off на AI-generation отдельно от marketing release | Да | Добавить LINE/KakaoMoment в media-mix; добавить bilingual AI DISCLOSURE overlay; запросить отдельный AI-generation release; добавить Korean-language disclosure для KR-target креативов |

8/8 PASS → промт выпускается ученику.
≤7/8 PASS → return to editor, не выпускается. Конкретные FAIL-причины перечисляются в return-note.

**APAC_PRESET — 3-STEP FUNNEL GRANULARITY:**

| Step | Цель | Арка | EXACT STRING focus + Tone | CTA | Регуляторный риск |
|------|------|------|---|-----|---|
| TOF | Awareness — educational content для cold APAC аудитории (skincare science JP, K-beauty trend KR, financial literacy SG, wellness habit AU) — общая категория, не product-specific | G/F через §19B EDUCATIONAL-PROGRESSION + ANIMATED-DIAGRAM | Country-appropriate register (keigo / jondaetmal / SG-English / AU-English) · нейтральный voice-over · cool-neutral palette JP/KR, warm AU, mixed SG | «무료 가이드 다운로드 / 無料ガイド / Download free guide / 免费指南» | Низкий (educational без product claim) |
| MOF | Nurture — founder / clinician / advisor Soul ID + brand POV + quality differentiator (Japanese craft sourcing, K-beauty patented active, SG MAS-licensed status, AU TGA-listed) | C — Soul ID tripod talking-head + §19A UI MOCKUP (ingredient panel / registration overlay) + §19B INFOGRAPHIC-CHART | Empathic peer (within register limits) · adapted-to-culture wardrobe · regulation overlay native + English | «상담 예약 / 無料相談予約 / Book consultation / 預約諮詢» | Средний (substantiation claim + APPI/PIPA/PDPA/Privacy Act Art.6 при retargeting + Yakuji Hō / MFDS / HSA / TGA disclaimer overlay) |
| BOF | Close — pre-qualified consultation, конкретный action на платную / purchase | C — Soul ID + product/service offering overlay + tax-inclusive pricing | Confident neutral · без давления · конкретно · pricing prominent | «今すぐ申し込む (税込¥XX,XXX) / 지금 신청하기 (₩XXX,XXX) / Subscribe now (S$XX incl GST) / Shop now (A$XX incl GST)» | Высокий (consumer protection acts + crypto/financial licensing + cooling-off period disclosure + Click-to-Cancel APAC equivalent) |

GUARDRAIL: Cold BOF CTA для регулируемых услуг (financial / health / cosmetic surgery referral / crypto) — высокий риск FSA / FSC / MAS / ASIC / Yakuji Hō / MFDS / HSA / TGA enforcement. Default — soft TOF/MOF в cold; BOF — только в retargeting / warm после consent collection + identity verification (KYC для financial / CKA для SG accredited investor).

CROSS-REFERENCES (не дублировать в этом пресете, использовать ссылками):
— **EU_RUSSIAN_DIASPORA_PRESET (волна Т.8, строки 3892-4087)** — sub-profile pattern (горизонтальный overlay поверх вертикали; AI DISCLOSURE pack pattern для regulated professional Soul ID; cross-border professional services guardrail аналог APAC ПРАВИЛО 5 medical / financial). Pattern переиспользован: «Если хотя бы один триггер → все правила обязательны + stricter rule при конфликте».
— **LATAM_PRESET (волна Т.13/T.14, если уже создан)** — sub-profile pattern brother (Brazil LGPD + Argentina PDPL + Mexico LFPDPPP). При наличии — cross-ref для unified APAC + LATAM emerging-market sub-profile inventory.
— **WELLNESS_HEALTH_RESTRICTED_USA_PRESET (волна Т.7)** — US-DSHEA structure-function approach **НЕ применяется напрямую в APAC** (см. ПРАВИЛО 5 disambig). APAC использует country-specific framework (Yakuji Hō / MFDS / HSA / TGA).
— **ECOM_IMPULSE_USA_PRESET (волна Т.9)** — FTC ROSCA / Click-to-Cancel pattern — APAC аналог в ПРАВИЛО 4 (Tokutei Shōtorihiki Hō cooling-off + KFTC + CCCS + ACCC stricter requirements).
— **HIGH_TICKET_PRO_SERVICES_PRESET ПРАВИЛО 8** + **REAL_ESTATE_EXPAT_USA_PRESET ПРАВИЛО 8** + **EU_RUSSIAN_DIASPORA_PRESET ПРАВИЛО 8** — AI DISCLOSURE pattern переиспользован в APAC ПРАВИЛО 8 (bilingual CJK + English overlay + country regulatory matrix).
— **Currency-таблица строки 404-418** — расширение в APAC ПРАВИЛО 7 для JPY (税込) / KRW VAT inclusive / SGD GST inclusive / AUD GST inclusive обязательной inclusive-pricing.
— **§11 Soul ID guardrails + §15 NEVER #23 meta-policy блокер + §19A UI MOCKUP + §19B INFOGRAPHIC-CHART + §21 EXECUTIVE-CALIBRATED HUMANIZATION** — универсальные guards, применимы поверх preset. §19A критически важен для CJK glyphs.
— **V17 (Жертва hook check) + V18 (substantiation) + V19-BIOCLAIM** — universal guards. V19-BIOCLAIM усиливается в APAC: Yakuji Hō / MFDS / HSA / TGA strictness > DSHEA.
— **CRISIS-AUDIT-LAYER C4 (combined-identifier risk)** — cross-ref для APPI / PIPA cross-border targeting risk из ПРАВИЛО 3.
— **DIASPORA-TONE GUIDANCE строки 3183-3193** — pattern «warm palette + confident posture + identity-respect» переиспользован для cultural register ПРАВИЛО 2 (особенно JP/KR honorific calibration).
...
═══════════════════════════════════════════════════
CRYPTO_WEB3_PRESET — отдельный пресет (волна Т.16 после Т.16-А создателя)
═══════════════════════════════════════════════════

Контекст: до волны Т.16 покрытие CRYPTO/WEB3 в SKILL.md ограничивалось одной строкой в Soul-ID matrix (5351 — «Crypto education / fintech consulting / wealth coaching → OK + V19 sign-off») и упоминанием «crypto educator» в FINANCE founder POV (строка 676). Это закрывало только Soul ID-слой и игнорировало восемь критических плоскостей криптоинфоповода: (1) Howey-test квалификация любого token-offering как security под US SEC §5 / §17(a); (2) MICA EU (Markets in Crypto Assets, действует с января 2025) с CASP authorization + white paper + ESMA notification как обязательным минимумом для любого таргета EU-аудитории; (3) UK FCA crypto promotions rules 2023 с s.21 FSMA gateway + 24h cool-off period + risk-warning prominence; (4) State Money Transmitter License matrix (47 штатов + NY BitLicense) для exchange / wallet / payments rail; (5) Celebrity-endorsement precedent (Kim Kardashian $1.26M settlement SEC 2022 / Logan Paul / Lindsay Lohan / DJ Khaled) + AI-deepfake hard-block; (6) Smart-contract audit substantiation (CertiK / OpenZeppelin / Trail of Bits / ConsenSys Diligence — public report URL + дата + scope + insurance coverage); (7) Pseudo-anon founder guardrail (OK для DeFi infrastructure / NOT OK для retail-facing custodial exchange); (8) Investment-return claim hard-block («guaranteed APY» / «100x» / «moon» — SEC §17(a) + state UDAP + FTC §5). Регуляторная сложность ближе к private banking, чем к ECOM, поэтому шаблон строится по образцу HIGH_TICKET_PRO_SERVICES_PRESET (9 правил + чек-лист 9/9 PASS + 3-step funnel). Этот пресет — единая точка истины для CRYPTO/WEB3 креатива.

**КОГДА АКТИВИРУЕТСЯ CRYPTO_WEB3_PRESET:**

Профиль клиента содержит флаг `vertical=crypto_exchange|token_issuance|nft_marketplace|defi_protocol|web3_saas|crypto_wallet|blockchain_analytics|staking_service|crypto_card`, ИЛИ любой из триггеров:
— Продукт = crypto exchange (Coinbase / Binance / Kraken-class), token issuance (ICO / IEO / IDO / fair launch), NFT marketplace (OpenSea / Magic Eden / Blur), DeFi protocol (Uniswap / Aave / Compound), Web3 SaaS-инфраструктура (Alchemy / Infura / Magic / Privy), crypto wallet (MetaMask / Phantom / Coinbase Wallet), blockchain analytics (Chainalysis / Arkham), staking-as-a-service (Lido / RocketPool), crypto card (Crypto.com Card / Binance Card).
— Hook / Dialogue / EXACT STRING содержит любую из формул: «APY», «yield», «staking rewards», «token sale», «mint», «airdrop», «whitelist», «presale», «100x», «to the moon», «buy the dip», «next Bitcoin», «backed by {VC / celebrity}».
— Soul ID = founder с CT (Crypto Twitter) personality / pseudo-anon founder (mask, voice modulation, generic avatar) / VC partner endorser (a16z / Paradigm / Sequoia / Polychain / Multicoin).
— Гео-триггер: US (SEC + CFTC + FinCEN + OFAC + 47 state MTL + NY BitLicense) / EU (MICA + CASP + ESMA + 5AMLD / 6AMLD + Travel Rule + DORA) / UK (FCA + s.21 FSMA + crypto promotions 2023) / Singapore (MAS Payment Services Act 2019 + DPT license) / Japan (FSA + JVCEA + Travel Rule) / Korea (Specific Financial Information Act 2020 + VASP registration) / Australia (AUSTRAC + ASIC) / Switzerland (FINMA + DLT Act 2021) / Canada (CSA Crypto Trading Platform framework).
— Substantiation claim на TVL / AUM в protocol / число пользователей / token-holder count / transaction volume / audit-pass.

Если хотя бы один триггер — все 9 правил ниже **обязательны**. Не «опционально к рассмотрению».

**ПРАВИЛО 1 — Workspace + модель + packs (дефолт жёсткий, не предложение):**
— Workspace = Cinema Studio (cinematic ICO / token-launch trailer / DeFi protocol manifest / Web3 thought-leadership с founder gravitas) ИЛИ Marketing Studio (founder talking-head explainer / quick product update / community AMA snippet).
— Модель = Veo 3.1 (для cinematic ICO / token launch / smart-contract visualizer / TVL chart с motion-graphics) ИЛИ Kling 3.0 (для founder talking-head 9-12s с lip-sync — особенно для CT-personality founder explainer).
— Packs = PORTRAIT CU (для founder face — 50-85mm, никогда wide-angle distort) + LIGHT CONSISTENCY (одна key-source через window-side + practical desk-lamp, не «hackathon neon» который читается как scam aesthetic) + FABRIC (зависит от positioning — hoodie + tee для DeFi-builder, suit для Web3 fintech B2B, sneaker-casual для Web3 SaaS founder) + ANTI-AI-LOOK (критично — crypto-аудитория hyper-sensitive к AI-tells после Sora rug-pull deepfakes 2024-2025) + REFLECTION (для glass / монитор с chart / metal logo) + HAIR (зависит от sub-vertical).
— UI / chart / TVL counter / token-price chart в кадре = ВСЕГДА через §19B INFOGRAPHIC-CHART / §19A UI MOCKUP PIPELINE с substantiation overlay (Dune / DefiLlama / Etherscan link), не напрямую в Veo генерируется fake-chart (AI-generated «portfolio gains» screenshot = automatic FAIL под Правило 2).
— Format = 9:16 + 9s (Meta primary, retargeting warm crypto-аудитории, X-cards), 16:9 + 12-15s (LinkedIn для VC / B2B Web3 SaaS / institutional DeFi positioning), 1:1 + 6-9s (X feed native).

**ПРАВИЛО 2 — Investment return claim hard-block (universal — automatic FAIL без обсуждения):**

Любая формулировка обещанной доходности / гарантированного APY / multiplier-claim в Hook / Dialogue / EXACT STRING / overlay / on-screen text / voice-over = automatic FAIL. Триггер кумулятивный — SEC §17(a) (anti-fraud в offer/sale of securities) + state UDAP (Unfair Deceptive Acts and Practices, 50 штатов) + FTC §5 (deceptive acts or practices) + UK FCA financial promotion rules + EU MICA market-abuse provisions.

Запрещённые формулы (любая = FAIL):
— «Guaranteed N% APY» / «Risk-free yield» / «Zero-loss staking» / «Stable N% return».
— «100x potential» / «1000x» / «next Bitcoin» / «next Ethereum» / «next Solana» / «moon» / «to the moon» / «moonshot».
— «Get rich» / «financial freedom» / «retire on crypto» / «replace your salary» / «passive income on autopilot».
— «Backed by Elon Musk / Sam Altman / Vitalik / CZ» без verifiable affiliation (см. Правило 7).
— «Decentralized · not a security» без Howey-test analysis (см. Правило 3) — самостоятельная декларация «not a security» НЕ работает как safe-harbor, регулятор смотрит на substance.
— Implied return через visual («$1k → $1M» chart, «from $5k to Lambo» arc) — visual implication считается claim под FTC и SEC.

PASS-замены:
— Вместо «Guaranteed 12% APY» → «Variable staking rewards, no fixed return, subject to validator performance and slashing risk — see {audit} and {whitepaper}».
— Вместо «100x potential» → удалить претензию полностью; альтернатива — «early-stage protocol with the usual associated risks» + ссылка на risk-disclosure лендинга.
— Вместо «backed by {celebrity}» → удалить или заменить на verifiable institutional investor с material connection disclosure (см. Правило 7).

FAIL пример: `Hook: "12% guaranteed APY on USDC — risk-free passive income · 100x potential on launch"`.
PASS пример: `Hook: "Variable staking rewards on USDC pool — no fixed return · validator slashing risk applies · audit by {CertiK report URL}, March 2026"`.

**ПРАВИЛО 3 — Howey-test analysis + US securities-law compliance для token sales (Reg D 506(c) / Reg A+ / Reg CF):**

Любое предложение токена US-резидентам / US-IP-targeting / English-language US-targeted социалка триггерит Howey-test (investment of money + common enterprise + expectation of profits + from efforts of others). Дефолт после SEC v. Ripple (2023) + SEC v. Coinbase (2023-2024) + SEC v. Binance (2023): любой token с promotion на secondary market + utility «через team execution» = security до явного no-action letter или Reg-exemption.

Триггер Правила 3 (любой = required compliance overlay):
— Token sale targeting US (явный или implicit через English-only без geo-fence).
— Pre-sale / private sale / SAFT (Simple Agreement for Future Tokens) аудитория US-резидентов.
— Secondary market промо с claim на «trade», «list», «exchange», «liquidity» — particularly для US-listed CEX (Coinbase, Kraken).

PASS-варианты (каждый требует substantiation):
— **Reg D 506(c)**: общий solicitation разрешён, но только accredited investor + verification (income $200k / net worth $1M / professional credential). Overlay в крео: «Sold only to accredited investors per Rule 506(c). Investor accreditation verified by {Verifier — Parallel Markets / VerifyInvestor.com / EarlyIQ} prior to allocation.» Form D filing reference на лендинге.
— **Reg A+ Tier 1 (до $20M / 12mo) / Tier 2 (до $75M / 12mo)**: SEC qualification statement обязательна. Overlay: «Offering qualified by SEC under Regulation A+ — Offering Circular at {SEC.gov/CIK/...}.»
— **Reg CF (до $5M / 12mo)**: только через FINRA-registered funding portal (StartEngine / Republic / Wefunder). Overlay: «Offered via {Portal} under Regulation Crowdfunding. Investment limits per Rule 100 apply.»
— **Non-US-only** (geo-fenced): overlay «Not available to US persons. Geo-restriction enforced.» + technical enforcement через KYC IP-block + sanctioned-list screening (Правило 9).

FAIL пример: `Hook: "Buy our token on the website — listing on top exchanges next month"` (US-visible, no exemption, secondary-market claim).
PASS пример: `Hook: "Strategic round open to accredited investors only — Rule 506(c) verification via Parallel Markets. Form D filed. See {Offering Memo}."`.

**ПРАВИЛО 4 — MICA EU compliance (CASP authorization + white paper + ESMA notification — required для любого EU-visible token offering после января 2025):**

MICA (Regulation (EU) 2023/1114) полностью применима с 30 декабря 2024 для CASP-сегмента, asset-referenced tokens — с июня 2024. Любой promo, видимый в EU (включая Meta lookalike в EU-странах, X-targeting EU-IP, EU-domain landing), требует:
— **CASP authorization** (Crypto Asset Service Provider) одной из 27 NCA (национальных competent authorities — BaFin DE, AMF FR, CONSOB IT, CNMV ES, MFSA MT, CySEC CY, AFM NL). Overlay: «Authorised as CASP by {NCA} — Authorisation N° {…}. Passportable across EU/EEA per MICA Article 65.»
— **White paper** под MICA Title II (utility token) / Title III (ART) / Title IV (EMT). Notification к NCA минимум 20 рабочих дней до публикации promo. Overlay-ссылка на white paper URL + дата ESMA-уведомления.
— **Marketing communications** под MICA Article 7 + Article 29: identifiable as marketing, consistent с white paper, fair / clear / not misleading. Risk warning prominence.
— **Travel Rule (Regulation 2023/1113)** для transfers между CASP — крео не может имитировать anonymous transfer без compliance overlay.
— **DORA (Regulation 2022/2554)** operational resilience: для CASP в крео нельзя обещать «100% uptime» / «no downtime» — это implicit DORA-compliance claim, который должен иметь substantiation.

Триггер обязательного MICA-overlay: язык крео = любой из EU24 ИЛИ targeting содержит EU-страну ИЛИ landing-page на .eu / .de / .fr / .it / .es / .nl / .pl / .pt / .gr / .cz / .se / .dk / .fi / .ie / .at / .be / .lu / .hu / .ro / .bg / .hr / .si / .sk / .ee / .lv / .lt / .mt / .cy.

FAIL пример: EU-targeted Meta-ad с «Buy {TOKEN} now — listing live» без CASP-overlay и без MICA white paper reference.
PASS пример: overlay «{Firm} authorised as CASP by BaFin (N° XX-XX). White paper at {URL}, notified to BaFin {date}. Marketing communication — not investment advice.»

**ПРАВИЛО 5 — UK FCA crypto promotions (FCA-registered firm + s.21 financial promotion approval + 24h cool-off period + risk-warning prominent):**

С 8 октября 2023 любой crypto financial promotion в UK (включая социалку с UK-target) требует:
— Issued / approved by **FCA-registered firm** (либо firm под temporary registration regime, либо S.21-approver).
— **s.21 FSMA gateway**: promo approval authorized person PRE-launch. Поле `s21_approver_name` + `approval_date` + `approval_reference` обязательны в intake.
— **Risk warning** — фиксированная формулировка FCA: «Don't invest unless you're prepared to lose all the money you invest. This is a high-risk investment and you are unlikely to be protected if something goes wrong. Take 2 mins to learn more.» Overlay в bottom-third, prominence ≥ benefit-claim prominence.
— **24-hour cooling-off period** для retail клиентов first-time: после appropriateness assessment + risk-summary + cooling-off, нельзя complete onboarding раньше 24h. Крео не может имитировать «sign up and buy now in seconds».
— **Personalised risk warning** + **client categorisation** (restricted / high-net-worth / sophisticated) до доступа.
— **Запрет incentive-to-invest**: «refer-a-friend $X», «sign-up bonus», «free token airdrop on deposit» — FCA конкретно flagged эти inducements в Policy Statement PS23/6.

FAIL пример: UK-target Instagram-ad «Sign up now — get 100 free {TOKEN} on first deposit».
PASS пример: overlay «Issued by {Firm} (FCA FRN XXXXXX). Approved as financial promotion by {S.21 Approver} on {date}. Don't invest unless you're prepared to lose all the money you invest. {…} Take 2 mins to learn more.»

**ПРАВИЛО 6 — State Money Transmitter License matrix (47 штатов + NY BitLicense — list licensing per state targeting):**

US crypto exchange / wallet / payments-rail / fiat-onramp = money-transmitter activity под BSA + state-level MTL regime. Targeting на конкретный штат без MTL = enforcement risk (CSBS coordinated actions 2020-2025).

Дефолтное правило: если крео taргетирует US без штатного исключения — substantiation должно включать перечень штатов с MTL ИЛИ overlay «Available in {list} — see {licenses page} for state availability» ИЛИ geo-fence enforcement.

— **NY BitLicense** (23 NYCRR Part 200) — отдельный регуляторный режим, не covered обычным MTL. Targeting NY-IP без BitLicense = NYDFS enforcement (Bittrex 2019, Robinhood Crypto 2023). Overlay (если NY включён): «BitLicense issued by NYDFS, License N° {…}.»
— **California DFPI Digital Financial Assets Law (DFAL)** — effective July 2026, registration required.
— **Texas / Florida / Washington / Illinois** — активные MTL enforcement, отдельные criteria для крипто-virtual currency.
— **Список доступных штатов** на лендинге обязателен + technical enforcement geo-block для нелицензированных.

Триггер: продукт = exchange / custodial wallet / fiat-onramp / staking-as-a-service для retail / crypto card / payments rail. Утилитарный non-custodial wallet (как MetaMask) — отдельная категория (MSB FinCEN registration + не MTL).

FAIL пример: US-broad targeting без state availability disclosure.
PASS пример: overlay «{Firm} licensed as money transmitter in 47 US states + DC. NY BitLicense N° {…}. State availability at {URL}. Not available in {excluded states}.»

**ПРАВИЛО 7 — Celebrity / influencer endorsement (SEC Kim Kardashian $1.26M settlement 2022 + Logan Paul + Lindsay Lohan + DJ Khaled precedents — material connection disclosure + AI deepfake hard-block):**

Любая celebrity / influencer / CT-personality, promoting crypto в крео, триггерит cumulative violations:
— **SEC §17(b) Securities Act** — disclosure of consideration для promotion of security. Kim Kardashian settled $1.26M (2022) за promo EthereumMax без disclosure. DJ Khaled / Floyd Mayweather settled $767K (2018) за Centra Tech ICO. Lindsay Lohan + 7 others — settled с SEC 2023 (Tronix promo). Logan Paul — civil litigation за CryptoZoo ($2.3M settlement 2025).
— **FTC §5 + Endorsement Guides 16 CFR Part 255** — material connection disclosure required, «#ad» / «#sponsored» / «#paid» visible в frame.
— **ASA UK CAP 3.18 + 3.19** + FCA financial promotion approval (если promo = crypto = automatic financial promotion под Правило 5).

Substantiation требования:
— Disclosure overlay на frame, не bottom-of-caption: «Paid promotion for {Firm}. Material connection: {founder equity / advisory token allocation / cash fee}.»
— Sign-off от celebrity на marketing use + AI-likeness use отдельно (если AI-генерация).
— **AI deepfake celebrity = hard-block без written license + EU AI Act Art. 50 transparency overlay**. Любой Soul ID, recognizable как реальная celebrity (Elon Musk / Vitalik / CZ / Saylor / Sam Altman) без written license = automatic FAIL + cease-and-desist risk + right-of-publicity tort (CA §3344 / NY §51).
— **VC endorsement** (a16z / Paradigm / Sequoia / Polychain / Multicoin / Pantera / Coinbase Ventures): использовать только если material connection disclosed AND VC permission granted AND verifiable в Crunchbase / VC public portfolio.

FAIL пример: AI-generated Vitalik Buterin face в Hook «I'm bullish on {TOKEN}».
PASS пример: real founder в frame с overlay «Founder, {Firm}. Material connection: equity holder + token allocation per Form D filing.»

**ПРАВИЛО 8 — Smart contract audit substantiation (CertiK / OpenZeppelin / Trail of Bits / ConsenSys Diligence — public report URL + дата + scope + insurance coverage):**

«Audited» без specifics = FAIL. Audit-claim — substantiation, не decoration.

PASS-формула — четыре поля:
1. **Audit firm name** — одна из tier-1 (CertiK / OpenZeppelin / Trail of Bits / ConsenSys Diligence / Halborn / Quantstamp / Sigma Prime / Spearbit). Tier-2 / self-audit / unnamed = FAIL.
2. **Public report URL** — link к published report (CertiK Skynet / GitHub repo / firm website case-study).
3. **Audit date + scope** — date ≤12 месяцев до промо ИЛИ overlay «re-audited {date} post {version}»; scope — какие контракты в perimeter (token contract / vault / governance / bridge), какие не покрыты.
4. **Findings disclosure** — severity counts (critical / high / medium / low / informational) + remediation status. «No critical / all high resolved» — acceptable; «pending issues» — must disclose.

Дополнительные substantiation-слои для serious crypto-claims:
— **Insurance**: cold storage insurance (Marsh / Lloyd's syndicates / Coincover) — coverage amount + policy reference + perils covered (hot wallet exclusion typical).
— **Bug bounty**: Immunefi / HackenProof program — max payout + scope + current status.
— **Formal verification**: Certora / Runtime Verification — proof URL + assumed model.
— **Bridge security**: если cross-chain bridge — extra audit overlay + post-Ronin/Wormhole/Nomad/Multichain hack-pattern disclosure (cross-chain bridges = highest-risk category — $3B+ lost 2021-2024).

Запрет: «battle-tested» без TVL-history; «institutional-grade» без named institutional custodian (BitGo / Anchorage / Fireblocks / Komainu); «100% safe» (никогда — кросс-ref Правило 2).

FAIL пример: «Audited and safe — your funds are protected.»
PASS пример: «Token contract + vault audited by CertiK, March 2026 — report at {URL}. 0 critical / 2 high resolved. Cold-storage insurance via Lloyd's syndicate, $100M coverage per policy {ref}.»

**ПРАВИЛО 9 — AI DISCLOSURE для crypto founder (CT personality public domain) + pseudo-anonymity guardrail (OK для DeFi infrastructure / NOT OK для retail-facing custodial exchange):**

**(a) AI DISCLOSURE pack для real-founder Soul ID:**
Если Soul ID = реальный founder с CT (Crypto Twitter) personality public domain (verifiable Twitter / X handle ≥5k followers, GitHub commits, conference talks) → AI DISCLOSURE pack обязателен. Overlay: «AI-assisted visual likeness of {Name}, {Title} at {Firm} — generated with consent. Original audio.» Sign-off на marketing ≠ sign-off на AI-generation — требуется отдельный AI-generation release (cross-ref HIGH_TICKET_PRO_SERVICES_PRESET Правило 8 GUARDRAIL #1).

**(b) Pseudo-anonymity guardrail (двойной режим):**
— **OK для DeFi infrastructure / open-source protocol / non-custodial wallet / governance DAO**: pseudo-anon founders приемлемы (исторический паттерн от Satoshi через 0xMaki, Trent.eth, до Tetranode). Substantiation — public commits + on-chain reputation + verifiable timeline. Overlay: «Pseudonymous founder — public commit history at {GitHub URL} since {date}. On-chain identity at {ENS / address}.»
— **NOT OK для retail-facing custodial exchange / wallet с custody / fiat-onramp / staking-as-a-service с pooled funds / payments rail**: pseudo-anon founder в крео для custodial product = automatic FAIL. Regulatory KYC + state MTL + NY BitLicense + EU MICA + FCA registration require named senior management, custodial без named principal = registered-impossibility + retail fraud-risk-signal (FTX, Celsius, Voyager precedents).

**(c) EU AI Act Art. 50 extraterritorial transparency:**
AI-generated content depicting real person, видимое в EU = required overlay disclosure. Применима даже если producer не EU-резидент. Cross-ref EU_RUSSIAN_DIASPORA_PRESET Правило 7 + HIGH_TICKET_PRO_SERVICES_PRESET Правило 8.

**(d) OFAC / sanctions screening для targeting + token-recipient:**
Crypto-specific — OFAC SDN list включает crypto-addresses (Lazarus Group, Tornado Cash addresses, Garantex, Hydra). Cumulative с EU (EU 269/2014 + 833/2014) / UK OFSI / Swiss SECO. Targeting не должно включать sanctioned-country IPs (RU / IR / KP / VE / CU + sectoral exclusions BY / SY). Технический enforcement — IP geo-block + KYC screening prior to token claim / wallet creation. Cross-ref EU_RUSSIAN_DIASPORA_PRESET Правило 4 (sanctions overlay) + HIGH_TICKET_PRO_SERVICES_PRESET Правило 9(a) (sanctions/OFAC/SECO geo-screening).

**(e) GDPR Art. 9 + crypto wallet address как PII:**
Wallet address linkable to person = personal data под GDPR Recital 30 + national DPA interpretations (CNIL FR 2018, BfDI DE 2022). Targeting через on-chain analytics (Chainalysis / Arkham / Nansen) + Meta lookalike = combined-identifier risk. Cross-ref CRISIS-AUDIT-LAYER C4 + EU_RUSSIAN_DIASPORA_PRESET Правило 3.

**CRYPTO_WEB3_PRESET — финальный чек-лист (9 проверок перед выдачей промта):**

| # | Проверка | PASS если | FAIL → |
|---|---|---|---|
| 1 | Cinema Studio / Marketing Studio + Veo 3.1 / Kling 3.0 + PORTRAIT CU + LIGHT CONSISTENCY + FABRIC + ANTI-AI-LOOK + REFLECTION + HAIR packs | Да | Переключить workspace, подключить недостающие packs, особенно ANTI-AI-LOOK (crypto-аудитория hyper-sensitive) |
| 2 | Investment-return claim absent: НЕТ «guaranteed APY», НЕТ «100x / moon / next Bitcoin», НЕТ visual-implied return-arc ($1k → $1M), НЕТ unverified celebrity-backing claim | Да | Заменить на «variable rewards / slashing risk / see audit and whitepaper»; удалить multiplier-фразы; убрать visual return-implication |
| 3 | US targeting → Reg D 506(c) с accredited investor verifier ИЛИ Reg A+ с qualified offering circular ИЛИ Reg CF через FINRA portal ИЛИ geo-fence «Not available to US persons» + technical enforcement | Да | Добавить exemption overlay + verifier reference + Form D / Offering Circular URL; или geo-fence |
| 4 | EU targeting → CASP authorisation overlay (NCA name + license N°) + white paper URL + ESMA / NCA notification date + MICA Art.7 fair-clear-not-misleading + DORA-claim substantiation | Да | Добавить CASP overlay; ссылку на white paper; date of notification; убрать «100% uptime» без DORA-evidence |
| 5 | UK targeting → FCA-registered firm + s.21 approver name + approval date + risk-warning prominent (fixed FCA text) + 24h cool-off respected + НЕТ inducement-to-invest | Да | Добавить FCA FRN + s.21 approver overlay + risk-warning fixed text; убрать sign-up bonus / refer-a-friend |
| 6 | State MTL matrix для US exchange / wallet / payments rail: list of states ИЛИ overlay «Available in {list}» + NY BitLicense если NY включён + technical geo-block для нелицензированных | Да | Добавить state availability overlay; запросить BitLicense N°; включить geo-block |
| 7 | Celebrity / influencer endorsement: material connection disclosure on-frame («Paid promotion · founder equity / token allocation / cash fee»); AI deepfake celebrity без written license — hard-block; VC endorsement только с verifiable affiliation | Да | Добавить material connection overlay; удалить AI-deepfake celebrity; удалить «backed by» без verification |
| 8 | Smart contract audit substantiation: named tier-1 firm + public report URL + audit date ≤12mo + scope + findings (severity counts) + insurance / bug bounty / formal verification если applicable; bridge — extra disclosure | Да | Запросить audit URL + scope + findings; добавить insurance reference; для bridge — additional overlay |
| 9 | AI DISCLOSURE pack (для real-founder CT personality) + pseudo-anon guardrail (OK для DeFi infra / FAIL для retail custodial) + OFAC + EU AI Act Art.50 overlay + GDPR + sanctions geo-screening | Да | Добавить AI DISCLOSURE; переключить custodial product на named principal; добавить geo-block + sanctions overlay |

9/9 PASS → промт выпускается ученику.
≤8/9 PASS → return to editor, не выпускается. Конкретные FAIL-причины перечисляются в return-note.

**CRYPTO_WEB3 — 3-STEP FUNNEL GRANULARITY:**

В отличие от плоской CTA-альтернативы и по аналогии с HIGH_TICKET_PRO_SERVICES_PRESET / CRISIS_EXPERT 3-STEP FUNNEL, для CRYPTO_WEB3 funnel-step разбивка обязательна — из-за asymmetric регуляторного риска по этапам воронки.

| Step | Цель | Арка | CTA | Регуляторный риск |
|------|------|------|-----|---|
| TOF | Educational — explain DeFi mechanic / explain blockchain primitive / market thesis | G через §19B EDUCATIONAL-PROGRESSION (process / data visualization / smart-contract диаграмма без лиц) | «Read our research» / «Subscribe to {Newsletter}» / «Get our crypto market outlook» | Низкий (educational, не financial promotion если без «buy / invest» CTA и без token-specific claim) |
| MOF | Trust — audit transparency + team identity + open-source verifiability + on-chain stats | C (named founder Soul ID, Правила 1-7) / F через §19B ANIMATED-DIAGRAM (TVL chart + audit-pass overlay) | «Read our audit report» / «View on-chain stats at DefiLlama» / «See our GitHub» / «Join our governance forum» | Средний (substantiation на TVL / audit-claim; AI DISCLOSURE если real-founder Soul ID) |
| BOF | Conversion — token sale / exchange sign-up / staking / NFT mint | C (founder Soul ID с full compliance stack — Правила 3 / 4 / 5 / 6 в зависимости от гео) | «Verify accreditation» (Reg D) / «Open account» (CASP-licensed) / «Sign up» (FCA-approved promotion) / «Mint» (NFT с royalty disclosure) | Высокий (full SEC / MICA / FCA / State MTL stack обязательна; AI DISCLOSURE + sanctions screening + KYC mandatory) |

GUARDRAIL: Cold BOF CTA для token sale в открытой социалке без US-exemption / EU CASP / UK s.21 approval = SEC / MICA / FCA enforcement risk. Default — soft TOF/MOF в cold-аудитории, BOF только в retargeting / whitelisted-warm с full geo-fence + KYC-precheck. Для US — BOF в cold feed без Reg D 506(c) accredited verification = general-solicitation violation. Для EU — BOF без CASP authorization после Jan 2025 = MICA Art. 59 violation. Для UK — BOF без s.21 approver = FSMA s.21 criminal offence (custodial sentence до 2 лет).

═══════════════════════════════════════════════════

---

## Cross-references и покрытие

**Что покрыл DRAFT (180+ строк, 9 правил + чек-лист 9/9 + 3-step funnel):**
1. Полная категориальная карта (exchange / token issuance / NFT marketplace / DeFi / Web3 SaaS / wallet / analytics / staking / crypto card) — Правило 1 + блок «КОГДА АКТИВИРУЕТСЯ».
2. Investment-return hard-block (APY / 100x / moon) с SEC §17(a) + state UDAP + FTC §5 — Правило 2.
3. Howey-test + Reg D 506(c) / Reg A+ / Reg CF для US token sales — Правило 3.
4. MICA EU + CASP + white paper + ESMA + Travel Rule + DORA — Правило 4.
5. UK FCA crypto promotions 2023 + s.21 FSMA + 24h cool-off + risk-warning — Правило 5.
6. State MTL matrix (47 штатов + NY BitLicense + CA DFAL 2026) — Правило 6.
7. Celebrity-endorsement precedent (Kim Kardashian / Logan Paul / Lindsay Lohan / DJ Khaled) + AI deepfake hard-block — Правило 7.
8. Smart-contract audit substantiation (CertiK / OpenZeppelin / Trail of Bits / ConsenSys Diligence) + insurance + bug bounty + bridge guardrail — Правило 8.
9. AI DISCLOSURE для CT-personality founder + pseudo-anon guardrail (DeFi infra OK / custodial NOT OK) + EU AI Act Art.50 + OFAC + GDPR wallet-as-PII — Правило 9.
10. Чек-лист 9/9 PASS + 3-step funnel (TOF educational / MOF audit + team / BOF compliance stack).

**Cross-references на existing material в SKILL.md:**
— **CRYPTO/WEB3 META категория (Т.13)** — точка входа. Этот пресет — extension META cross-ref в полноценный 9-правильный preset.
— **HIGH_TICKET_PRO_SERVICES_PRESET (волна Т.6)** — template pattern для regulatory-heavy preset (9 правил + чек-лист 9/9 + 3-step funnel). Crypto regulatory complexity ближе к private banking, чем к ECOM, поэтому шаблон тот же.
  — Правило 7 (celebrity material connection) переиспользует pattern из HIGH_TICKET Правила 8 (AI DISCLOSURE pack).
  — Правило 9 (OFAC / sanctions) расширяет HIGH_TICKET Правило 9(a).
  — 3-step funnel mirroring HIGH_TICKET 3-step funnel structure (TOF educational низкий риск / MOF substantiation средний / BOF financial promotion высокий).
— **EU_RUSSIAN_DIASPORA_PRESET (волна Т.8)** — overlap по двум слоям:
  — Sanctions screening (OFAC / EU 269 / 833 / 5n / UK OFSI / Swiss SECO) — Правило 9(d) этого preset cross-ref EU_RUSSIAN_DIASPORA Правило 4.
  — GDPR Art. 9 + special-category targeting (wallet address как PII / on-chain analytics + Meta lookalike combined identifier) — Правило 9(e) cross-ref EU_RUSSIAN_DIASPORA Правило 3.
  — Cyprus DPA / Malta MFSA — popular crypto-jurisdictions с активной диаспорной аудиторией.
— **CRISIS-AUDIT-LAYER C4 + C8** — combined-identifier risk + accreditation generalization паттерн.
— **§19B INFOGRAPHIC-CHART / §19A UI MOCKUP PIPELINE** — TVL chart / token-price / audit-pass overlay генерация (Правило 1).
— **§21 EXECUTIVE-CALIBRATED HUMANIZATION** — для Web3 fintech B2B / institutional DeFi founder.
— **V18 substantiation** — TVL / AUM / audit / insurance claims.
— **V19 sign-off** — founder Soul ID release + AI-generation release отдельно.
— **AI DISCLOSURE pack** — Правило 9(a) переиспользует pattern.
— **Meta-policy-checker** — отдельно проверить advertising standards categories (financial products, get-rich-quick).

═══════════════════════════════════════════════════

# T.17-A — PET_SUPPLIES_USA_LAYER + OTC_PHARMA_USA_LAYER (draft)

Волна Т.17, агент T.17-A. Два niche extension layer'а поверх существующих PRESET'ов.
НЕ полные PRESET — только дельта правил поверх базы.

---

## PET_SUPPLIES_USA_LAYER — extension поверх ECOM_IMPULSE_USA (волна Т.17 после Т.17-А создателя)

**База:** ECOM_IMPULSE_USA (физический товар, FTC §5 unfair/deceptive, §255 endorsements, Made in USA, atypicality, shipping/return disclosure, age-gating если применимо).
**Дельта:** ветеринарная вертикаль — корма, лакомства, добавки, CBD для животных, dental products.
**Примеры товаров:** Greenies dental treats, Hill's Science Diet, Blue Buffalo, Purina Pro Plan, Charlotte's Web Pet CBD, Honest Paws CBD, Cosequin joint supplement, Zesty Paws.

### Правило PET-1. FDA CVM compliance — категория товара (food / supplement / drug)

FDA Center for Veterinary Medicine (CVM) делит pet-товары на три regulatory bucket'а, и креатив обязан соответствовать заявленной категории:
- **Pet food / treat** — питание, no therapeutic claim. Greenies, Hill's, Blue Buffalo. Можно говорить «complete and balanced nutrition», нельзя «treats arthritis».
- **Supplement / nutraceutical** — Cosequin, Zesty Paws joint chews. Можно «supports joint health», нельзя «cures hip dysplasia».
- **Veterinary drug** — требует FDA approval (NADA / ANADA). Если креатив утверждает что product «treats / cures / prevents disease» — товар автоматически переклассифицируется в animal drug и нуждается в FDA approval.
Запрет в крео: глаголы treat / cure / prevent / diagnose в применении к ветеринарному диагнозу.
Допустимо: support / promote / maintain / помогает поддерживать.

### Правило PET-2. AAFCO labeling standards

Association of American Feed Control Officials — добровольный стандарт принятый де-факто всеми state feed control officials. Креатив с упаковкой / pack-shot должен показывать совместимый дизайн:
- **Guaranteed Analysis** панель (минимум protein / fat / fiber / moisture).
- **Ingredient list** в порядке убывания веса.
- **Feeding guidelines** по весу животного.
- **AAFCO statement** — либо «formulated to meet» либо «feeding tested» (последний сильнее). Жизненная стадия: growth / maintenance / all life stages.
- Запрет в крео: «human grade» без сертификации, «organic» без USDA Organic, «natural» без AAFCO определения.

### Правило PET-3. State veterinary board — unauthorized practice of veterinary medicine

Каждый штат имеет State Veterinary Board. Диагностика, лечение, prescription для животного — practice of veterinary medicine, требует state license (DVM / VMD). Креатив-бренд (non-vet) не может:
- Ставить диагноз («your dog has anxiety» как утверждение).
- Назначать дозу attached to symptom («give 2 chews if your dog limps»).
- Заявлять о treatment ветеринарного состояния (arthritis, hip dysplasia, cancer, separation anxiety как clinical entity).
Допустимо: «may help support dogs experiencing occasional joint stiffness» (structure/function, не disease).
Если креатив включает veterinarian on-camera — нужна live state license (см. PET-5).

### Правило PET-4. CBD pet — state patchwork matrix

Федерально: 2018 Farm Bill разрешил hemp-derived CBD (<0.3% THC) для людей, НО FDA до сих пор не одобрил CBD как ингредиент в pet food / supplements. CVM позиция: CBD в pet products = unapproved animal drug.
State patchwork:
- **CO / OR / WA** — толерантнее, CBD pet products продаются открыто.
- **CA** — AB-45 (2021) разрешил hemp в pet products при labeling.
- **NV** — ограничения на in-state CBD pet sales.
- **ID / SD / NE** — строгие, риск enforcement.
Крео для CBD pet (Charlotte's Web Pet, Honest Paws):
- Geo-targeting обязателен, исключить high-risk штаты или клеймер.
- Запрет на disease claims (anxiety, seizures, cancer как treatment).
- COA (Certificate of Analysis) — third-party lab test, THC <0.3%, no heavy metals / pesticides.
- Disclaimer: «not evaluated by FDA, not intended to diagnose, treat, cure, prevent any disease in animals».

### Правило PET-5. Veterinarian Soul ID + AVMA ad rules + state license verification

Если в креативе появляется человек заявленный как veterinarian («Dr. Smith, DVM recommends…»):
- **Soul ID матч** = state veterinary board registry. Каждый штат держит публичный реестр licensed veterinarians. Имя + DVM/VMD + state license # должны проверяться.
- **AVMA (American Veterinary Medical Association) Principles of Veterinary Medical Ethics** — раздел об advertising: запрещает misleading claims, гарантии результата, comparisons без substantiation, false credentials.
- Запрет: «board-certified» без подтверждения ABVS (American Board of Veterinary Specialties) recognition.
- Если используется stock footage «vet in coat» без реального лица — обязателен disclaimer «actor portrayal» (FTC §255).
- Если testimonial «my vet recommended» от end-user — это потребительский endorsement, регулируется §255 (см. PET-6), но дополнительно: бренд не должен давать понять что вет был оплачен / спонсирован, если такое было — material connection disclosure.

### Правило PET-6. Customer testimonial (FTC §255 + atypicality, weight loss / lifespan claims для животных)

Testimonial от pet owner («my dog lost 15 lbs in 3 months on this food») попадает под FTC Endorsement Guides §255:
- **Honest experience** — реальный опыт реального клиента.
- **Material connection** — раскрыть если owner получил free product / payment / discount.
- **Atypicality disclaimer** — «results not typical» больше НЕ достаточно. Нужно раскрыть generally expected result (что типично для большинства). Для weight loss / lifespan extension / dramatic recovery — обязательно.
- Запрет ветеринарных claims из уст owner'а («cured my dog's diabetes») — переходит в PET-3 unauthorized practice + PET-1 drug claim, даже если это слова потребителя, бренд несёт ответственность за публикацию.
- Before/after фото: реальные, не stock, не stretched, дата съёмки фиксируется.

---

## OTC_PHARMA_USA_LAYER — extension поверх WELLNESS_HEALTH_RESTRICTED_USA (волна Т.17 после Т.17-А создателя)

**База:** WELLNESS_HEALTH_RESTRICTED_USA (DSHEA структуры supplement, FDA disclaimer, FTC substantiation, claim ladder structure/function vs disease).
**Дельта:** OTC drug вертикаль — Monograph system отдельный от DSHEA, разные regulatory tracks.
**Примеры товаров:** Bengay, Aspercreme, Tiger Balm, Icy Hot (topical analgesic); Coppertone, Neutrogena Ultra Sheer, Banana Boat (sunscreen SPF); Robitussin, Mucinex, Delsym (cough); Claritin, Zyrtec, Benadryl OTC (allergy); Cortizone-10, Aveeno 1% (topical hydrocortisone); Children's Tylenol, Infants' Motrin (pediatric).

### Правило OTC-1. FDA OTC Monograph compliance (active ingredient × indication × dosage)

OTC drugs регулируются через Monograph System (CFR Title 21) — НЕ через DSHEA. Каждая категория (sunscreen, topical analgesic, antitussive, antihistamine, hydrocortisone) имеет свой monograph: список одобренных active ingredients, разрешённые indications, max dosage, обязательные warnings.
Креатив-аудит:
- Active ingredient в крео должен быть в monograph для заявленной категории (например menthol / methyl salicylate / camphor / capsaicin для external analgesic).
- Indication словесно ограничен monograph language («temporary relief of minor aches and pains of muscles and joints associated with simple backache, arthritis, strains, bruises, sprains» — Bengay стандарт).
- Запрет claims вне monograph: «heals», «cures arthritis», «restores cartilage».
- **Drug Facts panel** на pack-shot обязателен: Active ingredient → Purpose → Use → Warnings → Directions → Inactive ingredients.
- Disambig vs WELLNESS базы: supplement = DSHEA + structure/function + «not evaluated by FDA» disclaimer; OTC drug = Monograph + disease claim допустим в рамках monograph + Drug Facts panel.

### Правило OTC-2. SPF claim substantiation (FDA sunscreen monograph + UVA/UVB + water resistance)

Sunscreen — отдельный sub-monograph, особенно строгий после 2011 Final Rule:
- **SPF число** — измерено по FDA in vivo тестированию, на 10 субъектах минимум. Креатив с числом SPF 30 / 50 / 70+ — только если есть лабораторный test report.
- **Broad spectrum** label — protection и UVA, и UVB. Требует Critical Wavelength ≥370 nm. Без broad spectrum нельзя claim «protects against skin cancer / early skin aging».
- **Water resistance** — только «water resistant (40 minutes)» или «water resistant (80 minutes)». Запрет терминов «waterproof», «sweatproof», «sunblock» — все отозваны FDA в 2011.
- **SPF cap** — FDA proposed rule (не финальный) ограничивает label до SPF 60+, но действующий стандарт допускает выше при substantiation.
- Запрет в крео: «all-day protection» (нужна reapplication каждые 2 часа), «100% protection» (никогда не 100%).

### Правило OTC-3. Drug interaction + application site disclaimers (topical near eyes / mucous membranes)

Topical analgesics (Bengay, Aspercreme с lidocaine, capsaicin products) и topical hydrocortisone несут risk при mis-application:
- **Eyes / mucous membranes** — запрет применения, обязательный warning «do not get into eyes». Если креатив показывает application — не на лице, не у глаз.
- **Heating pad + topical** — не комбинировать (риск burns). Bengay-style warning «do not use with heating pad».
- **Broken / irritated skin** — не применять.
- **Children** — возрастные ограничения (см. OTC-6).
- **Aspirin-sensitive** — methyl salicylate (Bengay active) absorbed systemically, есть зафиксированные deaths от over-application у спортсменов. Warning обязателен.
- Lidocaine topical (Aspercreme with Lidocaine) — отдельный sub-monograph, ограничение по % concentration (4% OTC max).

### Правило OTC-4. Online Pharmacy Act compliance (если distribution через online pharmacy)

Ryan Haight Online Pharmacy Consumer Protection Act (2008) — федеральный закон, регулирует online distribution drugs:
- OTC drugs формально не требуют prescription, но если distribution идёт через online pharmacy с Rx infrastructure (Amazon Pharmacy, Walgreens online, CVS online) — требуется регистрация pharmacy с DEA + state pharmacy boards.
- Креатив с CTA «buy online» — landing page должен показывать licensed pharmacy badge (VIPPS / NABP .pharmacy domain).
- Запрет «no prescription needed» если в исходном marketing были references к Rx (риск misrepresentation).
- Direct-to-consumer OTC через Shopify / brand site — не online pharmacy, под Ryan Haight не попадает, но FTC + state consumer protection всё равно применимы.

### Правило OTC-5. State pharmacy boards — OTC vs Rx distinction + state restrictions

Каждый штат имеет State Board of Pharmacy. Хотя OTC по федеральному определению без prescription:
- **Behind-the-counter (BTC)** — некоторые OTC (pseudoephedrine — Sudafed) ограничены, ID required, log book, лимит по количеству (Combat Methamphetamine Epidemic Act 2005). Креатив для pseudoephedrine-containing продуктов — disclaimer «available behind the counter, ID required».
- **State-specific bans** — emergency contraception, codeine cough syrup статус варьируется. Codeine OTC доступен в нескольких штатах (была отдельная категория), сейчас фактически везде Rx — нельзя creative позиционировать codeine OTC.
- **Pharmacist counseling** — некоторые штаты требуют pharmacist sign-off на отпуск отдельных OTC, креатив не должен противоречить.
- Geo-targeting если product имеет state-level варианты.

### Правило OTC-6. Pediatric labeling (Children's / Infants' формулы + Reye's syndrome warning для aspirin)

Если product для kids (Children's Tylenol, Infants' Motrin, Children's Benadryl, Children's Mucinex):
- **Возрастная градация** на упаковке: under 2 years — «ask a doctor» обязательно для большинства категорий.
- **Dosing chart** по weight + age, не by spoon (риск overdose), milliliter dosing с приложенным dosing device.
- **Reye's syndrome warning** — aspirin (acetylsalicylic acid) запрещён детям и подросткам с fever (грипп / chickenpox). Обязательный warning на любом aspirin-containing продукте: «Children and teenagers who have or are recovering from chicken pox or flu-like symptoms should not use this product. If nausea, vomiting, or fever occur, consult a doctor because these symptoms could be an early sign of Reye's syndrome, a rare but serious illness». Креатив для aspirin-containing никогда не таргетится на parents детей с flu-like symptoms.
- **Cough/cold OTC для under 4 years** — FDA рекомендует не использовать. Креатив для antitussives / decongestants не должен показывать toddlers / infants.
- **Alcohol content** в детских liquid OTC — раскрытие обязательно.
- Pediatric креатив = automatic high scrutiny от FDA + state AGs + FTC COPPA если digital targeting.

---

## Cross-references + что покрыл

### PET_SUPPLIES_USA_LAYER — что покрыл
- FDA CVM трёхкатегорийная модель (food / supplement / drug) — Правило PET-1.
- AAFCO labeling (Guaranteed Analysis / ingredient / feeding / nutritional adequacy statement) — PET-2.
- State veterinary boards + unauthorized practice (диагноз / prescription / treatment) — PET-3.
- CBD pet patchwork (federal FDA + state matrix CO/NV/WA/CA/ID) + COA + disclaimer — PET-4.
- Veterinarian Soul ID + AVMA ad ethics + state license verification + actor portrayal disclosure — PET-5.
- Customer testimonial под FTC §255 + atypicality + запрет vet claims в устах owner'а — PET-6.

### PET_SUPPLIES_USA_LAYER — cross-reference на ECOM_IMPULSE_USA (база)
- Pack-shot правила, Made in USA для pet food sourcing, return policy disclosure, age-gating если CBD pet — наследуется из ECOM_IMPULSE_USA, не дублируется.
- FTC §5 unfair/deceptive, §255 endorsement framework — база. PET LAYER добавляет veterinary-specific overlay (PET-3 + PET-5).
- Atypicality для weight loss / результата — база §255, PET-6 расширяет на pet weight loss / lifespan / recovery.
- Subscribe-and-save для pet food (auto-ship) — наследуется из базовых ROSCA правил ECOM_IMPULSE_USA, не повторяется здесь.

### OTC_PHARMA_USA_LAYER — что покрыл
- FDA OTC Monograph (active × indication × dosage × Drug Facts panel) — OTC-1.
- SPF / UVA / UVB / Critical Wavelength / water resistance + запрещённая терминология — OTC-2.
- Drug interaction + application site warnings (eyes / mucous membranes / heating pad / methyl salicylate systemic absorption) — OTC-3.
- Ryan Haight Online Pharmacy Act + VIPPS / .pharmacy domain — OTC-4.
- State pharmacy boards + BTC pseudoephedrine + state-specific restrictions — OTC-5.
- Pediatric labeling + Reye's syndrome aspirin + under-4 cough/cold + dosing device — OTC-6.

### OTC_PHARMA_USA_LAYER — cross-reference на WELLNESS_HEALTH_RESTRICTED_USA (база) + disambig OTC monograph vs DSHEA

Главный disambig пункт волны Т.17 — OTC drug и dietary supplement идут разными regulatory tracks, путать в крео нельзя:

| Параметр | DSHEA supplement (база WELLNESS_HEALTH_RESTRICTED) | OTC Monograph drug (этот LAYER) |
|---|---|---|
| Regulatory framework | Dietary Supplement Health and Education Act 1994 | FDA OTC Monograph (CFR Title 21) |
| Pre-market approval | Нет (notification only для new dietary ingredient) | Monograph compliance (либо NDA для non-monograph) |
| Allowed claims | Structure/function only («supports joint health») | Disease claim в рамках monograph («temporary relief of arthritis pain») |
| Mandatory disclaimer | «This statement has not been evaluated by FDA. This product is not intended to diagnose, treat, cure, or prevent any disease» | Drug Facts panel (Active / Purpose / Use / Warnings / Directions / Inactive) |
| Active ingredient regulation | GRAS / NDI notification | Monograph-listed active с established dosage |
| Marketing scrutiny | FTC + FDA warning letters | FDA + FTC + state pharmacy boards |

Из WELLNESS_HEALTH_RESTRICTED_USA наследуется:
- Общий FTC substantiation framework (competent and reliable scientific evidence).
- Health-related testimonial rules §255 + atypicality.
- Geo-targeting infrastructure для state-level варьирующихся продуктов.

OTC_PHARMA_USA_LAYER не повторяет DSHEA правила — они в базе. LAYER добавляет Monograph-specific overlay (OTC-1, OTC-2, OTC-3), pharmacy distribution (OTC-4, OTC-5), pediatric (OTC-6).

---

**Конец draft T.17-A. Передача в T.17-B для review / cross-check на коллизии с WELLNESS_HEALTH_RESTRICTED + ECOM_IMPULSE базами.**
