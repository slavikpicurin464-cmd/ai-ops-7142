...
## Сессия 2026-05-24: рассинхроны + P0 + крео-правила (commits b740c30…ae2c218)

- **offer-generator** — подписаны две оси (параллельный тест по бюджету vs генерация-пул 6/16/20).
- **client-profile** — крео-объёмы выровнены по канону (6-10 / 16-30 / 30-60); «приоритетные сегменты» → «параллельный тест» (не приоритезация).
- **research-structurer** — дисциплина цитат (дословность / не инвертировать смысл / медуслуга≠БАД / свежесть факта) — закрытие P0 прогона 10 ниш.
- **higgsfield-prompt-generator** — дубль крео-правил текста и качества (синхрон с PROMPT-4): текст-в-кадре через GPT Image 2 / Nano (не «no text» в негатив), разметка + подложка, перенос (не рвать число+валюту), safe-zone 9:16, предмет продажи, дети-не-AI-лицо, видео-минимум/4K-по-запросу. Закрыт пробел Чат-5 visual-only.

Канон-правки (PROMPT-4, KONVEYER, BRIEFING и др.) — в отдельном репозитории курса, см. его CHANGELOG в 00-CANON-FROZEN.
...
## Сессия 2026-05-24/25 — харден гейтов + marketing-first + назови-продукт + ужатие higgsfield

Контекст: те же скилы в Claude Code (Opus) дают результат, а в Cowork (слабее модель + стале Knowledge) — «дно» на серой нутре; установленный плагин в CC тоже оказался стале-снапшотом (git-правки не доходят без reinstall).

- **Харден вызова скилов** (`00fbba6`): СТОП-протокол «генерь по правилам, не из головы» в offer-generator / ru-copywriter / ru-marketer / client-profile / schwartz-podhody; заполняемый GREY-CIS слот-шаблон в offer-generator; явный GREY-CIS LIGHT-TOUCH в quality-gate.
- **Marketing-first, без морализаторства** (`00fbba6`): GREEN/RED-блок в meta-policy-checker / quality-gate / ru-copywriter / offer-generator. Гейт = фабрикация / виктимизация / disease-cure / therapy-refusal / named-конкуренты; смелые claim'ы (результат / симптом / доход если проект подтверждает) НЕ флагать. Анти-фабрикация + EU/US (EFSA/FTC) сохранены в обоих регистрах.
- **«Назови ЧТО продаёшь» / тест «о чём угодно»** (`e10ce52`): offer-generator анти-паттерн 6 + ru-copywriter хуки. Оффер обязан делать ясным И результат, И продукт; голая боль без продукта И голые механики без результата = провал; «продукт» = единица ценности (товар / услуга / для инфобиза метод-обучение или лид-магнит). Нише-агностично.
- **Ужатие higgsfield-prompt-generator** (`f74f85d`): SKILL.md 1055 → 455 строк (раздутое ядро не регистрировалось в CC; реком. ≤500); объём → REFERENCE.md §11 (4711 → 5277); description ~1050 → 660 симв.; хард-гейт «выдаёт промпт, генерацию НЕ запускает без явного "генерь"» сохранён. Бэкап в `_archive/SKILL-pre-slim-2026-05-25.md`. ⚠ Метрика «SKILL.md 4568 строк» в таблице выше устарела (теперь 455).

Запушено: b72d600 → 00fbba6 → e10ce52 → f74f85d. CC увидит свежие скилы только после reinstall плагина.
