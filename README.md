...
Набор из **25 скилов** для перформанс-таргетолога CIS-except-RU + EU + USA. Срабатывают в Claude автоматически по русским / английским фразам — не нужно ничего вызывать руками. Помогают пройти весь маршрут с клиентом — разобрать нишу, собрать ресерч, придумать офферы, написать крео, **сгенерировать промпт под видео-нейросети**, проверить кабинет перед запуском, продиагностировать стату, написать отчёт.
...
### Подготовка проекта - разбор клиента и экономики, сбор сырья, сегменты и карта смыслов
...
### Стратегия и креативная разработка - офферы, подходы, производство крео
...
### Запуск, проверка, диагностика - запуск, аналитика, решения и следующая итерация
...
| `meta-launch-checklist` | **Блок 0 + Блок 0.5 + 9 технических блоков** чек-лист готовности кабинета перед запуском — Pixel, CAPI, домен, iOS, UTM + EU compliance + Stripe-CAPI |
| `campaign-diagnoser` | Диагностирует почему кампания плохо качает — проходит по 10 основным + 3 особым узлам отказа |
...
- **5 пре-валидаторов A.1-A.5** (абсолютное обещание, реальный бренд, BIOCLAIM, конвертация валюты не в USD, DSHEA-гейт).
